...
const OUT='artifacts/mantra46-16806-conservative-global-ab';
...
async function runOne(query,variant){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    if(variant){
      ...
      const capNew='const regionalCapacity16755=focusMode?80:Math.min(180,Math.max(80,coverageGroups16736.size*4));';
      ...
      if(body.split(gridOld).length-1!==1)throw new Error('spread grid owner mismatch');body=body.replace(gridOld,'const nx16783=24,ny16783=24;');
    ...
    if(body.split(call).length-1!==1)throw new Error('main call not found');body=body.replace(call,"window.__EARTHLINE_M46_16806_HY=hy;"+call);
  ...
  const row={query,variant:variant?'24x24-cap180-x4':'control',loadError,timedOut,pageErrors,elapsedMs:Date.now()-started,audit};rows.push(row);console.log('EARTHLINE_M46_16806 '+JSON.stringify(row));try{await page.screenshot({path:OUT+'/'+query.toLowerCase()+'-'+(variant?'variant':'control')+'.png',fullPage:false});}catch(_){ }await context.close();
}
for(const q of STATES){await runOne(q,false);await runOne(q,true)}
await browser.close();writeFileSync(OUT+'/results.json',JSON.stringify(rows,null,2));console.log('EARTHLINE_M46_16806_SUMMARY '+JSON.stringify(rows));const bad=rows.filter(r=>!r.audit||r.loadError||r.timedOut||r.pageErrors.length||Number(r.audit.coreMs)>15000||Number(r.audit.generated)!==Number(r.audit.visible)||Number(r.audit.unsafe)!==0||Number(r.audit.outside?.swales||0)!==0);if(bad.length)process.exitCode=1;
