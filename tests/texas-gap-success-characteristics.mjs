...
await page.route('**/*',async route=>{
 ...
 const repl=`    const gapAttempts16621=[];
    ...
    if(coarseGapPass16621&&gapAttempts16621.length){
      ...
      const gapAddedDetails16624=[];
      for(let gapAttemptIndex16624=0;gapAttemptIndex16624<gapAttempts16621.length;gapAttemptIndex16624++){
        const attempt16621=gapAttempts16621[gapAttemptIndex16624];
        ...
        const mp16624=s16621.segment[Math.floor((s16621.segment.length-1)/2)]||null;
        gapAddedDetails16624.push({attemptIndex:gapAttemptIndex16624,gridCenter:{x:Number(g16621.x),y:Number(g16621.y)},screenedGrid:{x:Number(s16621.x),y:Number(s16621.y)},nearestBefore:covered16621.length?Math.min(...covered16621.map(q16621=>Math.hypot(Number(q16621.x)-Number(s16621.x),Number(q16621.y)-Number(s16621.y)))):null,linePixels:Number(lineLengthPixels(hy,s16621.segment)),slope:Number(c16621.slope),acc:Number(c16621.acc),midLngLat:mp16624});
      ...
      window.EARTHLINE_GAP_FALLBACK_DETAILS_16624=gapAddedDetails16624;
    }
