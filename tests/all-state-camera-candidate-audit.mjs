...
await page.route('**/*',async route=>{
  ...
  if(req.isNavigationRequest()&&req.resourceType()==='document'&&req.url().startsWith(SITE_URL)){
    ...
    if(!focusMode&&hy&&hy.validityMask16584){`;
    cameraPatchMatches=body.split(needle).length-1;
    if(cameraPatchMatches!==1)throw new Error('camera candidate anchor expected once, found '+cameraPatchMatches);
...
console.log('EARTHLINE_ALL_STATE_CAMERA_CANDIDATE_PATCH '+JSON.stringify({cameraPatchMatches}));
...
await browser.close();
if(cameraPatchMatches!==1||rows.some(r=>!r.pass))process.exitCode=1;
