...
mkdirSync('artifacts/mantra46-16788-live',{recursive:true});
...
for(const query of CASES){
  ...
  try{
    await page.goto(BASE+'?earthline_m46_16788='+encodeURIComponent(query)+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
    await page.waitForSelector('#searchInput',{timeout:30000});
    const marker=await page.evaluate(()=>{const h=document.documentElement.outerHTML;return h.includes('coarseContourCells16788')&&h.includes('refinementTileBudget16782=Math.min(16,Math.max(8,Math.ceil(gapsBefore16780.length/3)))')&&h.includes('loadDEM(tile16780.b,32,32,5500');});
    if(!marker)throw new Error('live product does not contain EARTHLINE 16788 marker');
  ...
  try{await page.screenshot({path:`artifacts/mantra46-16788-live/${slug}.png`,fullPage:false});}catch(_){}
  ...
  results.push(row); console.log('EARTHLINE_M46_16788_LIVE '+JSON.stringify(row));
  await context.close();
}
await browser.close();
writeFileSync('artifacts/mantra46-16788-live/results.json',JSON.stringify(results,null,2));
if(results.some(r=>r.loadError||r.timedOut||r.lastError||r.pageErrors.length||Number(r.coreMs)>15000||Number(r.unsafe)!==0||Number(r.generated)!==Number(r.visible)||Number(r.boundaryOutside?.swales||0)!==0||r.rootPass!==true))process.exitCode=1;
