...
try{
  ...
  const result=await page.evaluate(()=>{
    ...
    const waterLayers=layers.filter(l=>{
      ...
      return /water|lake|reservoir|riverbank|ocean|sea/.test(tag);
    ...
    for(const l of layers){
      const tag=((l.id||'')+' '+(l['source-layer']||'')).toLowerCase();
      if(!/water|lake|reservoir|riverbank|ocean|sea/.test(tag))continue;
      if(l.source&&l['source-layer'])pairs.set(l.source+'|'+l['source-layer'],[l.source,l['source-layer']]);
    }
    for(const [source,sourceLayer] of pairs.values()){
      try{candidates.push(...(mp.querySourceFeatures(source,{sourceLayer})||[]));}catch(_){}
    }
    for(const [sourceId,sourceDef] of Object.entries(style.sources||{})){
      if(String(sourceDef?.type||'').toLowerCase()!=='vector')continue;
      try{candidates.push(...(mp.querySourceFeatures(sourceId,{sourceLayer:'water'})||[]));}catch(_){}
    }
    ...
    const parts=[];
    const addPoly=rings=>{if(!Array.isArray(rings)||!rings[0]?.length)return;const bbox=bboxRing(rings[0]);if(!bbox.every(Number.isFinite))return;parts.push({rings,bbox})};
    ...
    for(let li=0;li<lines.length;li++){
      const pts=(lines[li]||[]).filter(finite);
      for(let i=0;i<pts.length;i++){
        vertices++;let w=inStyleWater(pts[i]);if(w>=0){hits++;if(examples.length<20)examples.push({kind:'vertex',line:li,i,coord:pts[i],part:w})}
        if(i===0)continue;const a=pts[i-1],b=pts[i],span=Math.max(Math.abs(b[0]-a[0]),Math.abs(b[1]-a[1])),steps=Math.max(1,Math.ceil(span/.002));
        for(let k=1;k<steps;k++){samples++;const p=[a[0]+(b[0]-a[0])*k/steps,a[1]+(b[1]-a[1])*k/steps];w=inStyleWater(p);if(w>=0){hits++;if(examples.length<20)examples.push({kind:'segment',line:li,i,coord:p,part:w});break}}
      }
    }
    return {flowCount:flows.length,waterLayerIds:waterLayers.slice(0,80),waterLayerCount:waterLayers.length,candidateFeatures:candidates.length,polygonParts:parts.length,vertices,samples,styleWaterHits:hits,examples,status:String(document.getElementById('earthlineVermontStatus16147')?.textContent||'').trim(),preflight:window.EARTHLINE_REGIONAL_ATOMIC_PREFLIGHT_16329||null};
  });
  console.log('MANTRA38_VISIBLE_WATER '+JSON.stringify({result,errors:errors.slice(0,20)}));
  if(!result.preflight?.passed)process.exitCode=2;
