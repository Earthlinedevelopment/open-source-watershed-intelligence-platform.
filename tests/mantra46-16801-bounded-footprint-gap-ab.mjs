...
const OUT='artifacts/mantra46-16801-footprint-gap-ab';
...
async function runOne(query,variant){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    if(variant){
      ...
      const lateIf="if(unresolved16731.length){";
      if(body.split(lateIf).length-1!==1)throw new Error('late gate owner mismatch');
      body=body.replace(lateIf,"if(selected16731.length&&unresolved16731.length){");

      const lateSlice="const lateBins16741=unresolved16731.slice(0,4),lateStart16741=performance.now();";
      if(body.split(lateSlice).length-1!==1)throw new Error('late slice owner mismatch');
      body=body.replace(lateSlice,"const lateBins16741=unresolved16731.slice(0,2),lateStart16741=performance.now();");

      const lateDem="loadDEM(tile16741.b,96,96,8000,'late coverage-gap verification '+tile16741.id)";
      if(body.split(lateDem).length-1!==1)throw new Error('late DEM owner mismatch');
      body=body.replace(lateDem,"loadDEM(tile16741.b,32,32,5500,'late coverage-gap verification '+tile16741.id)");

      const lateCount="for(const f16741 of swales.features||[]){const c16741=f16741&&f16741.geometry&&f16741.geometry.type==='LineString'?f16741.geometry.coordinates:null;if(!Array.isArray(c16741)||!c16741.length)continue;const mid16746=c16741[Math.floor((c16741.length-1)/2)],g16746=llGrid(hy,mid16746);if(!g16746||!Number.isFinite(g16746.x)||!Number.isFinite(g16746.y))continue;const bx16746=Math.max(0,Math.min(binsX16731-1,Math.floor(Number(g16746.x)*binsX16731/Math.max(1,hy.w)))),by16746=Math.max(0,Math.min(binsY16731-1,Math.floor(Number(g16746.y)*binsY16731/Math.max(1,hy.h)))),row16746=after16731.find(r16741=>r16741.bx===bx16746&&r16741.by===by16746);if(row16746)row16746.swales++;}";
      const lateCountNew="for(const f16741 of swales.features||[]){const c16741=f16741&&f16741.geometry&&f16741.geometry.type==='LineString'?f16741.geometry.coordinates:null;if(!Array.isArray(c16741)||!c16741.length)continue;for(const k16740 of earthlineCoverageBinsForLine16740(c16741)){const p16740=k16740.split(','),bx16740=Number(p16740[0]),by16740=Number(p16740[1]),row16740=after16731.find(r16741=>r16741.bx===bx16740&&r16741.by===by16740);if(row16740)row16740.swales++;}}";
      if(body.split(lateCount).length-1!==1)throw new Error('late footprint owner mismatch');
      body=body.replace(lateCount,lateCountNew);
      ...
      const selectNew=`const gapMap16801=new Map(gaps16731.map(g16801=>[g16801.bx+','+g16801.by,g16801])),seen16801=new Set();
      for(const gap16801 of gaps16731){
        const k016801=gap16801.bx+','+gap16801.by;if(seen16801.has(k016801))continue;
        const q16801=[gap16801],component16801=[];seen16801.add(k016801);
        while(q16801.length){
          const cur16801=q16801.shift();component16801.push(cur16801);
          for(let oy16801=-1;oy16801<=1;oy16801++)for(let ox16801=-1;ox16801<=1;ox16801++){
            if(!ox16801&&!oy16801)continue;
            const nx16801=cur16801.bx+ox16801,ny16801=cur16801.by+oy16801,k16801=nx16801+','+ny16801,n16801=gapMap16801.get(k16801);
            if(!n16801||seen16801.has(k16801))continue;seen16801.add(k16801);q16801.push(n16801);
          }
        }
        for(const c16801 of component16801)c16801.component16801=component16801.length;
      }
      const adaptivePool16801=gaps16731.filter(g16801=>Number(g16801.component16801||0)>=20);
      const selected16731=[];
      while(adaptivePool16801.length&&selected16731.length<4){
        let bestI16801=0,bestM16801=-Infinity;
        for(let i16801=0;i16801<adaptivePool16801.length;i16801++){
          const g16801=adaptivePool16801[i16801],spread16801=selected16731.length?Math.min(...selected16731.map(s16801=>Math.hypot(s16801.bx-g16801.bx,s16801.by-g16801.by))):4;
          const m16801=Number(g16801.component16801||0)*1000+spread16801*80+(g16801.opportunity/Math.max(1,g16801.valid))*100+g16801.preferred;
          if(m16801>bestM16801){bestM16801=m16801;bestI16801=i16801;}
        }
        selected16731.push(adaptivePool16801.splice(bestI16801,1)[0]);
      }`;
  ...
  try{
    await page.goto(BASE+'?m46_16801_ab='+encodeURIComponent(query)+'_'+(variant?'variant':'control')+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
  ...
  const row={query,variant:variant?'bounded-footprint-adaptive':'control-6x6',loadError,timedOut,pageErrors,elapsedMs:Date.now()-started,audit};
  rows.push(row);console.log('EARTHLINE_M46_16801_AB '+JSON.stringify(row));
...
console.log('EARTHLINE_M46_16801_AB_SUMMARY '+JSON.stringify(rows));
