...
const URL='https://earthlinedevelopment.org/?earthline_regression=tx_product_final_'+Date.now();
...
for(let run=0;run<CASES.length&&!loadError;run++){
  ...
  results.push(r);console.log('EARTHLINE_TX_PRODUCT_FINAL '+JSON.stringify(r));
}
await browser.close();
