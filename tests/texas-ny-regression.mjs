...
const URL='https://earthlinedevelopment.org/?earthline_regression=tx_cap80_product_'+Date.now();
...
for(let run=0;run<CASES.length&&!loadError;run++){
  ...
  const r={run:run+1,query,clickToTerminalMs:terminalAt-clickStarted,timedOut,coreMs:Number(after?.perf?.totalMs||Infinity),terrainSource:after?.perf?.terrainSource||null,phases:after?.perf?.phaseTotalsMs||null,candidates:after?.generation?.candidates??null,chosenBeforeTierGate:after?.generation?.chosenBeforeTierGate??null,beforeClip:after?.boundary?.before?.swales??null,afterClip:after?.boundary?.after?.swales??null,removedByBoundary:after?.boundary?.removed?.swales??null,generated:after?.pub?.generated??null,sourceFeatures:after?.pub?.sourceFeatures??null,overlaySwaleLines:after?.pub?.overlaySwaleLines??null,renderedCorridors:after?.label?.renderedCorridors??null,gridW:after?.flow?.gridAudit?.grid?.w??null,gridH:after?.flow?.gridAudit?.grid?.h??null,unsafeSegments:after?.flow?.unsafeSegments??null,status:after?.status||'',lastError:after?.lastError||null,pageErrors:pageErrors.slice(0,20)};
  results.push(r);console.log('EARTHLINE_TX_CAP80_PRODUCT '+JSON.stringify(r));
}
...
const txBad=tx.some(r=>Number(r.chosenBeforeTierGate)<80||Number(r.afterClip)<35||Number(r.generated)<35||Number(r.overlaySwaleLines)<28||Number(r.sourceFeatures)!==Number(r.generated))||new Set(tx.map(r=>r.generated)).size!==1;
if(loadError||results.length!==CASES.length||commonBad||txBad)process.exitCode=1;
