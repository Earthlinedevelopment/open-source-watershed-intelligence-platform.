...
const URL='https://earthlinedevelopment.org/?earthline_regression=tx_final_gate_'+Date.now();
...
for(let run=0;run<CASES.length&&!loadError;run++){
  ...
  const r={run:run+1,query,clickToTerminalMs:terminalAt-clickStarted,timedOut,coreMs:Number(after?.perf?.totalMs||Infinity),terrainSource:after?.perf?.terrainSource||null,slowestPhase:after?.perf?.slowestPhase||null,phases:after?.perf?.phaseTotalsMs||null,status:after?.status||'',generated:after?.pub?.generated??null,sourceFeatures:after?.pub?.sourceFeatures??null,overlaySwaleLines:after?.pub?.overlaySwaleLines??null,overlayLocationPills:after?.pub?.overlayLocationPills??null,renderedCorridors:after?.label?.renderedCorridors??null,renderedLabels:after?.label?.renderedLabels??null,gridW:after?.flow?.gridAudit?.grid?.w??null,gridH:after?.flow?.gridAudit?.grid?.h??null,unsafeSegments:after?.flow?.unsafeSegments??null,lastError:after?.lastError||null,pageErrors:pageErrors.slice(0,20)};
  results.push(r);console.log('EARTHLINE_TX_FINAL_GATE '+JSON.stringify(r));
}
await browser.close();
writeFileSync('tx-ny-regression-results.json',JSON.stringify(results,null,2));
const tx=results.filter(r=>r.query==='Texas');
const bad=loadError||results.length!==CASES.length||results.some(r=>r.timedOut||r.lastError||!Number.isFinite(r.coreMs)||r.coreMs>15000||r.clickToTerminalMs>15000||!/screening published\./i.test(r.status)||Number(r.gridW)!==96||Number(r.gridH)!==96||Number(r.unsafeSegments)!==0||(Number(r.generated||0)>0&&Number(r.overlaySwaleLines||0)===0)||(r.sourceFeatures!=null&&r.generated!=null&&Number(r.sourceFeatures)!==Number(r.generated)))||new Set(tx.map(r=>r.generated)).size!==1||new Set(tx.map(r=>r.overlaySwaleLines)).size!==1;
