...
const OUT='artifacts/mantra46-16798-anchor-relaxed-fallback';
...
for(const query of STATES){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    const oldAnchor="const c=sampleSegment(coords,idx,false);\n          if(c){c.spatialAnchor16786=true;c.spatialAnchorCell16786=key;candidates.push(c);supplementalAnchorCandidates16786++;}";
    const newAnchor="const c=sampleSegment(coords,idx,false)||sampleSegment(coords,idx,true);\n          if(c){c.spatialAnchor16786=true;c.spatialAnchorCell16786=key;candidates.push(c);supplementalAnchorCandidates16786++;}";
    if(body.split(oldAnchor).length-1!==1)throw new Error('supplemental spatial-anchor sampler owner not found');
    body=body.replace(oldAnchor,newAnchor);

    const oldCapture="window.EARTHLINE_SWALE_GENERATION_AUDIT_16167=generationAudit;";
    const newCapture="window.__EARTHLINE_M46_16798_CHAIN={hy,candidates,chosen};window.EARTHLINE_SWALE_GENERATION_AUDIT_16167=generationAudit;";
