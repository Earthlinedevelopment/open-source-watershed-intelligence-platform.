...
const OUT='artifacts/mantra46-16799-adaptive-gap-ab';
...
async function runOne(query,variant){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    if(variant){
      ...
      const selectNew=`const gapMap16799=new Map(gaps16731.map(g16799=>[g16799.bx+','+g16799.by,g16799])),seen16799=new Set();
      for(const gap16799 of gaps16731){
        const k016799=gap16799.bx+','+gap16799.by;if(seen16799.has(k016799))continue;
        const q16799=[gap16799],component16799=[];seen16799.add(k016799);
        while(q16799.length){
          const cur16799=q16799.shift();component16799.push(cur16799);
          for(let oy16799=-1;oy16799<=1;oy16799++)for(let ox16799=-1;ox16799<=1;ox16799++){
            if(!ox16799&&!oy16799)continue;
            const nx16799=cur16799.bx+ox16799,ny16799=cur16799.by+oy16799,k16799=nx16799+','+ny16799,n16799=gapMap16799.get(k16799);
            if(!n16799||seen16799.has(k16799))continue;seen16799.add(k16799);q16799.push(n16799);
          }
        }
        for(const c16799 of component16799)c16799.component16799=component16799.length;
      }
      const adaptivePool16799=gaps16731.filter(g16799=>Number(g16799.component16799||0)>=20).sort((a16799,b16799)=>{
        const cd16799=Number(b16799.component16799||0)-Number(a16799.component16799||0);if(cd16799)return cd16799;
        const ar16799=a16799.opportunity/Math.max(1,a16799.valid),br16799=b16799.opportunity/Math.max(1,b16799.valid);
        return (br16799*100+b16799.preferred*2+b16799.opportunity/4)-(ar16799*100+a16799.preferred*2+a16799.opportunity/4);
      });
      const selected16731=[];
      while(adaptivePool16799.length&&selected16731.length<4){
        let bestI16799=0,bestM16799=-Infinity;
        for(let i16799=0;i16799<adaptivePool16799.length;i16799++){
          const g16799=adaptivePool16799[i16799],spread16799=selected16731.length?Math.min(...selected16731.map(s16799=>Math.hypot(s16799.bx-g16799.bx,s16799.by-g16799.by))):4;
          const m16799=Number(g16799.component16799||0)*1000+spread16799*80+(g16799.opportunity/Math.max(1,g16799.valid))*100+g16799.preferred;
          if(m16799>bestM16799){bestM16799=m16799;bestI16799=i16799;}
        }
        selected16731.push(adaptivePool16799.splice(bestI16799,1)[0]);
      }`;
  ...
  try{
    await page.goto(BASE+'?m46_16799_ab='+encodeURIComponent(query)+'_'+(variant?'variant':'control')+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
  ...
  const row={query,variant:variant?'adaptive-24x24-4x32':'control-6x6',loadError,timedOut,pageErrors,elapsedMs:Date.now()-started,audit};
  rows.push(row);console.log('EARTHLINE_M46_16799_AB '+JSON.stringify(row));
...
console.log('EARTHLINE_M46_16799_AB_SUMMARY '+JSON.stringify(rows));
