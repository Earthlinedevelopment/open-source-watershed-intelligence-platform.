...
const OUT='artifacts/mantra46-16801-capacity-ab';
...
async function runOne(query,variant){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    if(variant){
      ...
      const neu='const regionalCapacity16755=focusMode?80:Math.min(240,Math.max(80,coverageGroups16736.size*8));';
      const count=body.split(old).length-1;
      if(count!==1)throw new Error('capacity owner count='+count);
    ...
    body=body.replace(call,"window.__EARTHLINE_M46_CAP_HY=hy;"+call);
  ...
  try{
    await page.goto(BASE+'?m46_16801='+encodeURIComponent(query)+'_'+(variant?'expanded':'control')+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
    ...
    try{
      await page.waitForFunction(q=>{
        ...
        return /screening published\./i.test(s)&&!!window.__EARTHLINE_M46_CAP_HY&&root&&String(root.query||'').toLowerCase().includes(q.toLowerCase());
  ...
  const audit=loadError||timedOut?null:await page.evaluate(q=>{
    const hy=window.__EARTHLINE_M46_CAP_HY;
    ...
    return {
      ...
      candidates:gen?.candidates??null,
      eligible:gen?.jurisdictionEligibleCandidates??null,
      chosen:gen?.chosenBeforeTierGate??null,
      capacity:spatial?.capacity??null,
  ...
  const row={query,variant:variant?'expanded-capacity':'control',loadError,timedOut,pageErrors,elapsedMs:Date.now()-started,audit};
  rows.push(row);console.log('EARTHLINE_M46_16801_AB '+JSON.stringify(row));
  try{await page.screenshot({path:OUT+'/'+query.toLowerCase()+'-'+(variant?'expanded':'control')+'.png',fullPage:false});}catch(_){ }
  await context.close();
}
