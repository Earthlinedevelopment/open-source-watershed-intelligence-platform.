...
for(let repeat=1;repeat<=4;repeat++){
  ...
  await page.route('**/*',async route=>{
    ...
    apply('projectionAudit16634',
      'const swaleCount=swales.features.length;',
      `const swaleCount=swales.features.length;
      try{
        const lengths16634=[];
        for(const f16634 of (swales&&swales.features||[])){
          const coords16634=f16634&&f16634.geometry&&f16634.geometry.type==='LineString'?f16634.geometry.coordinates:[];
          let prev16634=null,len16634=0,finite16634=0;
          for(const ll16634 of coords16634){
            try{const p16634=m.project({lng:Number(ll16634[0]),lat:Number(ll16634[1])});if(p16634&&Number.isFinite(p16634.x)&&Number.isFinite(p16634.y)){finite16634++;if(prev16634)len16634+=Math.hypot(p16634.x-prev16634.x,p16634.y-prev16634.y);prev16634=p16634;}}catch(_){}
          }
          lengths16634.push({len:Number(len16634.toFixed(3)),finite:finite16634});
        }
        const bb16634=m.getBounds&&m.getBounds(),cc16634=m.getCenter&&m.getCenter(),host16634=document.querySelector('.map-area')||document.getElementById('mapboxBase'),canvas16634=m.getCanvas&&m.getCanvas(),sorted16634=lengths16634.map(x=>x.len).sort((a,b)=>a-b);
        window.EARTHLINE_RENDER_PROJECTION_16634={zoom:Number(m.getZoom&&m.getZoom()),center:cc16634?{lng:Number(cc16634.lng),lat:Number(cc16634.lat)}:null,bounds:bb16634?[bb16634.getWest(),bb16634.getSouth(),bb16634.getEast(),bb16634.getNorth()]:null,host:{w:Number(host16634&&host16634.clientWidth||0),h:Number(host16634&&host16634.clientHeight||0)},canvas:{w:Number(canvas16634&&canvas16634.clientWidth||0),h:Number(canvas16634&&canvas16634.clientHeight||0),width:Number(canvas16634&&canvas16634.width||0),height:Number(canvas16634&&canvas16634.height||0)},count:lengths16634.length,min:sorted16634[0]??null,p10:sorted16634[Math.floor(sorted16634.length*.1)]??null,median:sorted16634[Math.floor(sorted16634.length*.5)]??null,p90:sorted16634[Math.floor(sorted16634.length*.9)]??null,max:sorted16634[sorted16634.length-1]??null,ge4:lengths16634.filter(x=>x.len>=4).length,ge8:lengths16634.filter(x=>x.len>=8).length,ge12:lengths16634.filter(x=>x.len>=12).length,ge22:lengths16634.filter(x=>x.len>=22).length,renderReturn:regionalOverlayRenderOk16336,displayAudit:window.EARTHLINE_REGIONAL_DISPLAY_AUDIT_16040||null,publicationView:window.EARTHLINE_REGIONAL_PUBLICATION_VIEW_16337||null};
      }catch(error16634){window.EARTHLINE_RENDER_PROJECTION_16634={error:String(error16634)};}`);
  ...
  await page.goto(URL+'?tx_singlepass='+repeat+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
