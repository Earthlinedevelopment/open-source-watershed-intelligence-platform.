...
const OUT='artifacts/mantra46-16798-cluster-gap-ab';
...
async function runOne(query,variant){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    if(variant){
      ...
      for(const [oldv,newv,expected] of repl){
        ...
        if(count!==expected)throw new Error('threshold owner mismatch '+oldv+' count='+count);
      ...
      const densityNew=`const earthlineCoverageDensityTarget16767=r16767=>1;`;
      if(body.split(density).length-1!==1)throw new Error('density owner mismatch');
      body=body.replace(density,densityNew);
      ...
      const selectNew=`for(const gap16798 of gaps16731){
        gap16798.cluster16798=gaps16731.filter(o16798=>Math.hypot(o16798.bx-gap16798.bx,o16798.by-gap16798.by)<=1.5).length;
      }
      gaps16731.sort((a16798,b16798)=>{
        const cd16798=Number(b16798.cluster16798||0)-Number(a16798.cluster16798||0);
        if(cd16798)return cd16798;
        const ar16798=a16798.opportunity/Math.max(1,a16798.valid),br16798=b16798.opportunity/Math.max(1,b16798.valid);
        return (br16798*100+b16798.preferred*2+b16798.opportunity/4)-(ar16798*100+a16798.preferred*2+a16798.opportunity/4);
      });
      const selected16731=[];for(const gap16731 of gaps16731){if(selected16731.length>=8)break;selected16731.push(gap16731);}`;
  ...
  try{
    await page.goto(BASE+'?m46_16798_ab='+encodeURIComponent(query)+'_'+(variant?'variant':'control')+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
  ...
  const row={query,variant:variant?'24x24-cluster-8':'control-6x6',loadError,timedOut,pageErrors,elapsedMs:Date.now()-started,audit};
  rows.push(row);console.log('EARTHLINE_M46_16798_AB '+JSON.stringify(row));
...
console.log('EARTHLINE_M46_16798_AB_SUMMARY '+JSON.stringify(rows));
