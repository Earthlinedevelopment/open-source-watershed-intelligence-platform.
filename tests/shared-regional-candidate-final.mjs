...
for(const testCase of CASES){
  ...
  const row={
    ...
    phases:a?.perf?.phaseTotalsMs??null,
    candidates:a?.generation?.candidates??null,
    eligible:a?.generation?.jurisdictionEligibleCandidates??null,
    chosen:a?.generation?.chosenBeforeTierGate??null,
    published:a?.pub?.generated??null,
    visible:a?.display?.swaleLines??null,
    gridW:a?.flow?.gridAudit?.grid?.w??null,
    gridH:a?.flow?.gridAudit?.grid?.h??null,
    unsafe:a?.flow?.unsafeSegments??null,
    removed:a?.boundary?.removed??null,
    outsideAfterClip:a?.boundary?.outsideAfterClip??null,
  ...
  results.push(row);
  console.log('EARTHLINE_FINAL_SHARED_GATE '+JSON.stringify(row));
  await context.close();
}

await browser.close();
writeFileSync('shared-regional-candidate-final.json',JSON.stringify(results,null,2));

const failed=results.some(r=>
  r.loadError||r.timedOut||r.lastError||r.pageErrors.length||
  Number(r.gridW)!==96||Number(r.gridH)!==96||Number(r.unsafe)!==0||
  Number(r.published)<Number(r.minPublished)||Number(r.visible)<Number(r.minPublished)||
  Number(r.clickToTerminalMs)>15000||
  Number(r.outsideAfterClip?.contours||0)!==0||
  Number(r.outsideAfterClip?.flows||0)!==0||
  Number(r.outsideAfterClip?.swales||0)!==0
);
