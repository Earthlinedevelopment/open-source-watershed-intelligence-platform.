...
const COMBOS=[[112,96],[112,100],[112,104],[112,108]];
...
for(const [upper,lower] of COMBOS){
  ...
  await page.route('**/*',async route=>{
    ...
    const needle=`const d16702=await loadDEM(tile16702.b,112,112,6000,'scale refinement '+tile16702.id)`;
    const repl=`const localRes16706=tile16702.id==='coast-1'?${upper}:${lower},d16702=await loadDEM(tile16702.b,localRes16706,localRes16706,6000,'scale refinement '+tile16702.id)`;
    patchMatches=body.split(needle).length-1;
    if(patchMatches!==1)throw new Error('hybrid local refinement anchor expected once, found '+patchMatches);
    body=body.replace(needle,repl);
    return route.fulfill({response:resp,body});
  });
  await page.goto(URL+'?tx_refine_hybrid='+upper+'x'+lower+'&t='+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
  ...
  for(let repeat=1;repeat<=2;repeat++){
    ...
    const row={upper,lower,repeat,elapsedMs:Date.now()-started,timedOut,patchMatches,snap};rows.push(row);console.log('EARTHLINE_TX_REFINE_HYBRID '+JSON.stringify(row));
  }
  await page.close();
}
console.log('EARTHLINE_TX_REFINE_HYBRID_SUMMARY '+JSON.stringify(rows));
await browser.close();
