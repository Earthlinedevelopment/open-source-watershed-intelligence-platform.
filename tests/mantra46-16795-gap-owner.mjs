...
const OUT='artifacts/mantra46-16795-gap-owner';
...
for(const query of STATES){
  ...
  try{
    await page.goto(BASE+'?m46_gapowner='+encodeURIComponent(query)+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
  ...
  const audit=loadError||timedOut?null:await page.evaluate(()=>{
    ...
    const candidateCover=new Set(),contourCover=new Set(),selectedCover=new Set();
    ...
    for(const f of contours?.features||[]){
      if(f?.geometry?.type==='LineString')coverLine(f.geometry.coordinates,contourCover);
    }

    const noCandidate=[...qualified].filter(k=>!candidateCover.has(k));
    const noCandidateWithContour=noCandidate.filter(k=>contourCover.has(k));
    const noCandidateNoContour=noCandidate.filter(k=>!contourCover.has(k));
    const selected12=new Set((fine?.selected||[]).map(r=>r.bx+','+r.by));
    const refinedParent=k=>{
      const [x,y]=k.split(',').map(Number);
      return selected12.has(Math.floor(x/2)+','+Math.floor(y/2));
    };
    return {
      qualified:qualified.size,
      candidates:(candidates||[]).length,
      chosen:(chosen||[]).length,
      contourCovered:contourCover.size,
      ...
      noCandidateNoContour:noCandidateNoContour.length,
      noCandidateRefinedParent:noCandidate.filter(refinedParent).length,
      noCandidateUnrefinedParent:noCandidate.filter(k=>!refinedParent(k)).length,
      noCandidateWithContourKeys:noCandidateWithContour,
      noCandidateNoContourKeys:noCandidateNoContour,
      fineSelected:[...(fine?.selected||[])].map(r=>({bx:r.bx,by:r.by}))
    };
  });

  if(audit){
    audit.withContourClusters=clusters(audit.noCandidateWithContourKeys).map(c=>c.length);
    audit.noContourClusters=clusters(audit.noCandidateNoContourKeys).map(c=>c.length);
    delete audit.noCandidateWithContourKeys;delete audit.noCandidateNoContourKeys;
  }
  const row={query,loadError,timedOut,pageErrors,elapsedMs:Date.now()-started,audit};
  rows.push(row);console.log('EARTHLINE_M46_GAP_OWNER '+JSON.stringify(row));
  await context.close();
}
await browser.close();
writeFileSync(OUT+'/results.json',JSON.stringify(rows,null,2));
console.log('EARTHLINE_M46_GAP_OWNER_SUMMARY '+JSON.stringify(rows));
