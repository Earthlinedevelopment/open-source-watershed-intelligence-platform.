...
let capPatchMatches=0;
await page.route('**/*',async route=>{
  if(route.request().resourceType()!=='document')return route.continue();
  const resp=await route.fetch();let body=await resp.text();
  const needle='      if(chosen.length>=80)break;';
  capPatchMatches=body.split(needle).length-1;
  if(capPatchMatches!==1)throw new Error('cap100 expected one selection-cap match, found '+capPatchMatches);
  body=body.replace(needle,'      if(chosen.length>=100)break;');
  return route.fulfill({response:resp,body});
});
await page.goto(SITE_URL+'?all_state_cap100='+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
...
for(const stateName of STATES){
  ...
  rows.push(row);
  console.log('EARTHLINE_CAP100_STATE '+JSON.stringify(row));
}

console.log('EARTHLINE_CAP100_SUMMARY '+JSON.stringify({cap:100,capPatchMatches,
  states:rows.length,
