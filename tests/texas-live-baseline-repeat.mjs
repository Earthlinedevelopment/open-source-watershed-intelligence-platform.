...
for(let repeat=1;repeat<=3;repeat++){
  ...
  const state=await page.evaluate(()=>{
    ...
    const p=window.EARTHLINE_REGIONAL_PERFORMANCE_16191||null,g=window.EARTHLINE_SWALE_GENERATION_AUDIT_16167||null,d=window.EARTHLINE_REGIONAL_DISPLAY_AUDIT_16040||null;
    return {swales:sw.length,visible:d?.swaleLines??null,candidates:g?.candidates??null,eligible:g?.jurisdictionEligibleCandidates??null,totalMs:p?.totalMs??null,phases:p?.phaseTotalsMs??null,lastError:window.EARTHLINE_LAST_LIVE_REGIONAL_ERROR_15970||null};
  });
...
console.log('EARTHLINE_TX_BASELINE_REPEAT_SUMMARY '+JSON.stringify(rows));
await browser.close();
