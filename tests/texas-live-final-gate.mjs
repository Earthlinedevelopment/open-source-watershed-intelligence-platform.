...
`    const productsStarted16198=performance.now();
    const txProfile16649={runToken,startedAt:productsStarted16198,useSupplemental:false};
    window.EARTHLINE_TX_PRODUCT_PROFILE_16649=txProfile16649;
    let txMark16649=performance.now();`);
...
`    txMark16649=performance.now();
    let contours=await makeContours(hy);
    txProfile16649.normalContoursMs=Math.round(performance.now()-txMark16649);
    noteRegionalProgress16347('DERIVING WATER PATHS + BIOSWALES · CONTOURS COMPLETE',runToken);
    txMark16649=performance.now();await wait(0);txProfile16649.postNormalContoursYieldMs=Math.round(performance.now()-txMark16649);`);

  apply('boundaryAwait',
`      selectionBoundary16539=await jurisdictionBoundaryPromise16539;`,
`      txMark16649=performance.now();
      selectionBoundary16539=await jurisdictionBoundaryPromise16539;
      txProfile16649.boundaryAwaitMs=Math.round(performance.now()-txMark16649);`);

  apply('supplementalContours',
`    const swaleCandidateContours16609=useSupplemental16609?await makeContours(hy,true):contours;`,
`    txProfile16649.useSupplemental=!!useSupplemental16609;
    txMark16649=performance.now();
    const swaleCandidateContours16609=useSupplemental16609?await makeContours(hy,true):contours;
    txProfile16649.supplementalContoursMs=Math.round(performance.now()-txMark16649);`);
...
`    txMark16649=performance.now();
    ...
    txProfile16649.swalesMs=Math.round(performance.now()-txMark16649);
    noteRegionalProgress16347('DERIVING WATER PATHS + BIOSWALES · SWALES COMPLETE',runToken);
    txMark16649=performance.now();await wait(0);txProfile16649.postSwalesYieldMs=Math.round(performance.now()-txMark16649);`);

  apply('styleWater',
`      hy.styleWaterIndex16584=earthlineLoadedStyleWaterIndex16584(map(),hy.bounds);`,
`      txMark16649=performance.now();
      hy.styleWaterIndex16584=earthlineLoadedStyleWaterIndex16584(map(),hy.bounds);
      txProfile16649.styleWaterIndexMs=Math.round(performance.now()-txMark16649);`);
...
`    txMark16649=performance.now();
    ...
    txProfile16649.flowsMs=Math.round(performance.now()-txMark16649);
    noteRegionalProgress16347('DERIVING WATER PATHS + BIOSWALES · FLOWS COMPLETE',runToken);
    txMark16649=performance.now();await wait(0);txProfile16649.postFlowsYieldMs=Math.round(performance.now()-txMark16649);
    txMark16649=performance.now();`);
...
`    txProfile16649.flowAuditMs=Math.round(performance.now()-txMark16649);
    window.EARTHLINE_ORB_RESPONSIVENESS_16245={build:'EARTHLINE 16245',cooperativeTerrain:true,compositorOrb:true,productsMs:Math.round(performance.now()-contoursStarted16245),at:new Date().toISOString()};`);
...
`      txMark16649=performance.now();
      const bounded16539=earthlineClipRegionalProducts16539({contours,flows,swales,query:q,runToken},boundary16539);
      txProfile16649.jurisdictionClipMs=Math.round(performance.now()-txMark16649);`);
...
`    const cameraFinalStarted16329=performance.now();
    const cameraReady16334=await cameraSettle16310;
    phase16198.cameraReadyBeforePublishMs=Math.round(performance.now()-cameraFinalStarted16329);
    txProfile16649.cameraFinalMs=phase16198.cameraReadyBeforePublishMs;`);

  apply('finalWaterClip',
`      flows=earthlineFinalWaterClip16584(hy,flows,m,runToken);`,
`      txMark16649=performance.now();
      flows=earthlineFinalWaterClip16584(hy,flows,m,runToken);
      txProfile16649.finalWaterClipMs=Math.round(performance.now()-txMark16649);`);
...
`    txProfile16649.publicationMs=Math.round(performance.now()-txMark16649);
    txProfile16649.totalProductsMs=Math.round(performance.now()-productsStarted16198);
    txProfile16649.completedAt=performance.now();
    window.EARTHLINE_TX_PRODUCT_PROFILE_16649=Object.assign({},txProfile16649);
    phaseMark16198('terrainProductsAndPublishMs',productsStarted16198);`);

  return route.fulfill({response:resp,body});
});
await page.goto(URL+'?tx_compute_yield_confirm16649='+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
...
for(let repeat=1;repeat<=5;repeat++){
 ...
 const state=await page.evaluate(()=>{
  ...
  return {swales:sw.length,visible:d?.swaleLines??null,published:g?.publishedFeatures??null,totalMs:p?.totalMs??null,phases:p?.phaseTotalsMs??null,profile:window.EARTHLINE_TX_PRODUCT_PROFILE_16649||null,gridW:flow?.gridAudit?.grid?.w??null,gridH:flow?.gridAudit?.grid?.h??null,unsafe:flow?.unsafeSegments??null,outside:b?.outsideAfterClip??null,lastError:window.EARTHLINE_LAST_LIVE_REGIONAL_ERROR_15970||null,zones:{panhandleNorth:count((x,y)=>x>-103.1&&x<-100&&y>35&&y<36.6),upperCoast:count((x,y)=>x>-96.5&&x<-93.45&&y>28.8&&y<31.2),lowerCoast:count((x,y)=>x>-99.5&&x<-97&&y>25.7&&y<28.2),eastInterior:count((x,y)=>x>-96&&x<-93.45&&y>30.5&&y<34.3)}};
 });
 const row={repeat,elapsedMs:Date.now()-started,timedOut,state};rows.push(row);console.log('EARTHLINE_TX_PRODUCT_PROFILE_16649 '+JSON.stringify(row));
}
console.log('EARTHLINE_TX_PRODUCT_PROFILE_16649_SUMMARY '+JSON.stringify({patches,rows}));
await browser.close();
