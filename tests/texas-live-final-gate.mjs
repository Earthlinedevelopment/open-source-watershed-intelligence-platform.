...
const rows=[],cases=['Texas','Texas','Texas','New York','Vermont','Massachusetts'];
for(let ci=0;ci<cases.length;ci++){const repeat=ci+1,query=cases[ci];
 ...
 const row={repeat,query,elapsedMs:Date.now()-started,timedOut,state};rows.push(row);console.log('EARTHLINE_TX_CHUNKED_UI '+JSON.stringify(row));
}
console.log('EARTHLINE_TX_CHUNKED_UI_SUMMARY '+JSON.stringify({patches,rows}));
await browser.close();
const commonBad=rows.some(r=>r.timedOut||r.state.lastError||!(r.state.totalMs<=15000)||Number(r.state.visible||0)<=0||Number(r.state.published||0)<=0||Number(r.state.unsafe)!==0||Number(r.state.outside?.swales||0)!==0);
...
if(commonBad||txBad||texas.length!==3)process.exitCode=1;
