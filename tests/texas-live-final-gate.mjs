...
`    const productsStarted16198=performance.now();
    const txProfile16650={runToken,startedAt:productsStarted16198,useSupplemental:false};
    window.EARTHLINE_TX_PRODUCT_PROFILE_16650=txProfile16650;
    let txMark16650=performance.now();`);
...
`    txMark16650=performance.now();
    let contours=await makeContours(hy);
    txProfile16650.normalContoursMs=Math.round(performance.now()-txMark16650);
    noteRegionalProgress16347('DERIVING WATER PATHS + BIOSWALES · CONTOURS COMPLETE',runToken);
    txMark16650=performance.now();await new Promise(requestAnimationFrame);txProfile16650.postNormalContoursYieldMs=Math.round(performance.now()-txMark16650);`);

  apply('boundaryAwait',
`      selectionBoundary16539=await jurisdictionBoundaryPromise16539;`,
`      txMark16650=performance.now();
      selectionBoundary16539=await jurisdictionBoundaryPromise16539;
      txProfile16650.boundaryAwaitMs=Math.round(performance.now()-txMark16650);`);

  apply('supplementalContours',
`    const swaleCandidateContours16609=useSupplemental16609?await makeContours(hy,true):contours;`,
`    txProfile16650.useSupplemental=!!useSupplemental16609;
    txMark16650=performance.now();
    const swaleCandidateContours16609=useSupplemental16609?await makeContours(hy,true):contours;
    txProfile16650.supplementalContoursMs=Math.round(performance.now()-txMark16650);`);
...
`    txMark16650=performance.now();
    ...
    txProfile16650.swalesMs=Math.round(performance.now()-txMark16650);
    noteRegionalProgress16347('DERIVING WATER PATHS + BIOSWALES · SWALES COMPLETE',runToken);
    txMark16650=performance.now();await new Promise(requestAnimationFrame);txProfile16650.postSwalesYieldMs=Math.round(performance.now()-txMark16650);`);

  apply('styleWater',
`      hy.styleWaterIndex16584=earthlineLoadedStyleWaterIndex16584(map(),hy.bounds);`,
`      txMark16650=performance.now();
      hy.styleWaterIndex16584=earthlineLoadedStyleWaterIndex16584(map(),hy.bounds);
      txProfile16650.styleWaterIndexMs=Math.round(performance.now()-txMark16650);`);
...
`    txMark16650=performance.now();
    ...
    txProfile16650.flowsMs=Math.round(performance.now()-txMark16650);
    noteRegionalProgress16347('DERIVING WATER PATHS + BIOSWALES · FLOWS COMPLETE',runToken);
    txMark16650=performance.now();await new Promise(requestAnimationFrame);txProfile16650.postFlowsYieldMs=Math.round(performance.now()-txMark16650);
    txMark16650=performance.now();`);
...
`    txProfile16650.flowAuditMs=Math.round(performance.now()-txMark16650);
    window.EARTHLINE_ORB_RESPONSIVENESS_16245={build:'EARTHLINE 16245',cooperativeTerrain:true,compositorOrb:true,productsMs:Math.round(performance.now()-contoursStarted16245),at:new Date().toISOString()};`);
...
`      txMark16650=performance.now();
      const bounded16539=earthlineClipRegionalProducts16539({contours,flows,swales,query:q,runToken},boundary16539);
      txProfile16650.jurisdictionClipMs=Math.round(performance.now()-txMark16650);`);
...
`    const cameraFinalStarted16329=performance.now();
    const cameraReady16334=await cameraSettle16310;
    phase16198.cameraReadyBeforePublishMs=Math.round(performance.now()-cameraFinalStarted16329);
    txProfile16650.cameraFinalMs=phase16198.cameraReadyBeforePublishMs;`);

  apply('finalWaterClip',
`      flows=earthlineFinalWaterClip16584(hy,flows,m,runToken);`,
`      txMark16650=performance.now();
      flows=earthlineFinalWaterClip16584(hy,flows,m,runToken);
      txProfile16650.finalWaterClipMs=Math.round(performance.now()-txMark16650);`);
...
`    txProfile16650.publicationMs=Math.round(performance.now()-txMark16650);
    txProfile16650.totalProductsMs=Math.round(performance.now()-productsStarted16198);
    txProfile16650.completedAt=performance.now();
    window.EARTHLINE_TX_PRODUCT_PROFILE_16650=Object.assign({},txProfile16650);
    phaseMark16198('terrainProductsAndPublishMs',productsStarted16198);`);

  return route.fulfill({response:resp,body});
});
await page.goto(URL+'?tx_frame_yield16650='+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
