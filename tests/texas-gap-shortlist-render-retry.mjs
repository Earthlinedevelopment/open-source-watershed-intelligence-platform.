 ...
 const repl=`    const gapAttempts16621=[];
    ...
    if(coarseGapPass16621&&gapAttempts16621.length){
      ...
      for(const attempt16621 of gapAttempts16621){
        ...
        let nearest16623=Infinity;
        for(const q16621 of covered16621){
          const d16623=Math.hypot(Number(q16621.x)-Number(g16621.x),Number(q16621.y)-Number(g16621.y));
          if(d16623<nearest16623)nearest16623=d16623;
        }
        if(nearest16623<8)continue;
        shortlist16623.push({attempt:attempt16621,distance:nearest16623,gx:g16621.x,gy:g16621.y});
      }
      shortlist16623.sort((a16623,b16623)=>b16623.distance-a16623.distance);
      let gapEvaluated16621=0;
      for(const item16623 of shortlist16623.slice(0,24)){
        if(gapAdded16621>=12)break;
        gapEvaluated16621++;
        const c16621=sampleSegment(item16623.attempt[0],item16623.attempt[1],false,4);if(!c16621)continue;
      ...
      window.EARTHLINE_GAP_FALLBACK_SHORTLIST_16623={eligible:shortlist16623.length,evaluated:gapEvaluated16621,limit:24};
...
`    const regionalOverlayRenderOk16336=
      typeof window.earthlineRenderRegionalOverlay16020==='function'
        ...
        :false;`,
`    let regionalOverlayRenderOk16336=
      typeof window.earthlineRenderRegionalOverlay16020==='function'
        ?window.earthlineRenderRegionalOverlay16020(visualData)
        :false;
    if(!regionalOverlayRenderOk16336&&swales&&Array.isArray(swales.features)&&swales.features.length){
      try{m.resize&&m.resize();}catch(_){}
      regionalOverlayRenderOk16336=
        typeof window.earthlineRenderRegionalOverlay16020==='function'
          ?window.earthlineRenderRegionalOverlay16020(visualData)
          :false;
      window.EARTHLINE_REGIONAL_RENDER_RETRY_16623={runToken,attempted:true,success:!!regionalOverlayRenderOk16336,at:new Date().toISOString()};
    }`);
