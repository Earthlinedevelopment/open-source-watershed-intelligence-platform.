...
mkdirSync('artifacts/mantra46-16787-live',{recursive:true});
...
for(const query of CASES){
  ...
  try{
    await page.goto(BASE+'?earthline_m46_16787='+encodeURIComponent(query)+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
    await page.waitForSelector('#searchInput',{timeout:30000});
    const marker=await page.evaluate(()=>document.documentElement.outerHTML.includes('EARTHLINE 16787 — EFFICIENT TERRAIN-GAP RECOVERY')&&document.documentElement.outerHTML.includes('loadDEM(tile16780.b,32,32,5500'));
  ...
  try{await page.screenshot({path:`artifacts/mantra46-16787-live/${slug}.png`,fullPage:false});}catch(_){}
  ...
  results.push(row); console.log('EARTHLINE_M46_16787_LIVE '+JSON.stringify(row));
  await context.close();
}
await browser.close();
writeFileSync('artifacts/mantra46-16787-live/results.json',JSON.stringify(results,null,2));
