...
const OUT='artifacts/mantra46-16800-anchor-success-quota';
...
for(const query of STATES){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    const newFixed="        const fixedKey16800=!focusMode?anchorKey16786(coords,idx):null;\n        const c=sampleSegment(coords,idx,false);\n        if(c){candidates.push(c);if(!focusMode)noteAnchor16786(fixedKey16800);}";
    ...
    const newRule="rule:'preserve existing fixed anchors, then give each 12x12 contour cell up to two successful real makeSwales candidates; failed samples do not consume the cell quota; capacity and science gates unchanged',";
    ...
    body=body.replace(genAudit,"window.__EARTHLINE_M46_16800_CHAIN={hy,candidates,chosen};"+genAudit);
  ...
  const audit=loadError||timedOut?null:await page.evaluate(()=>{
    const {hy,candidates,chosen}=window.__EARTHLINE_M46_16800_CHAIN;
  ...
  rows.push(row);
  console.log('EARTHLINE_M46_16800 '+JSON.stringify(row));
