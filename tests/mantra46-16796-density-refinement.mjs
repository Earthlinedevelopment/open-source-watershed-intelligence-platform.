...
const OUT='artifacts/mantra46-16796-density-refinement';
...
for(const query of STATES){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    const marker="countFineSwales16780();";
    const insert=`
      const earthlineFineDensityTarget16796=r16796=>{
        const preferred16796=Number(r16796&&r16796.preferred||0);
        if(preferred16796>=15)return 3;
        if(preferred16796>=8)return 2;
        return 1;
      };
    `;
    const markerCount=body.split(marker).length-1;
    if(markerCount<2)throw new Error('expected fine-swale recount markers, found '+markerCount);
    body=body.replace(marker,marker+insert);

    const oldGap="return r16780.valid>=20&&r16780.opportunity>=8&&ratio16780>=.18&&r16780.swales===0;";
    const newGap="return r16780.valid>=20&&r16780.opportunity>=8&&ratio16780>=.18&&r16780.swales<earthlineFineDensityTarget16796(r16780);";
    const gapCount=body.split(oldGap).length-1;
    if(gapCount!==2)throw new Error('expected two fine gap predicates, found '+gapCount);
    body=body.split(oldGap).join(newGap);

    const oldMetric="const metric16781=(row16780.coarseContourHit16788?0:10000)+pref16781*1200+ratio16781*180+row16780.preferred*6+spread16781*24+(Number(row16780.cluster16781)||0)*12+row16780.opportunity/16;";
    const newMetric="const densityDeficit16796=Math.max(0,earthlineFineDensityTarget16796(row16780)-Number(row16780.swales||0));const metric16781=densityDeficit16796*5000+(row16780.coarseContourHit16788?0:10000)+pref16781*1200+ratio16781*180+row16780.preferred*6+spread16781*24+(Number(row16780.cluster16781)||0)*12+row16780.opportunity/16;";
    if(body.split(oldMetric).length-1!==1)throw new Error('fine selection metric owner not found');
    body=body.replace(oldMetric,newMetric);

    const oldCapture="window.EARTHLINE_SWALE_GENERATION_AUDIT_16167=generationAudit;";
    const newCapture="window.__EARTHLINE_M46_16796_CHAIN={hy,candidates,chosen};window.EARTHLINE_SWALE_GENERATION_AUDIT_16167=generationAudit;";
  ...
  try{
    await page.goto(BASE+'?m46_16796='+encodeURIComponent(query)+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
    ...
    try{
      await page.waitForFunction(q=>{
        ...
        return /screening published\./i.test(s)&&!!window.__EARTHLINE_M46_16796_CHAIN&&root&&String(root.query||'').toLowerCase().includes(q.toLowerCase());
  ...
  const audit=loadError||timedOut?null:await page.evaluate(()=>{
    const {hy,candidates,chosen}=window.__EARTHLINE_M46_16796_CHAIN;
  ...
  rows.push(row);
  console.log('EARTHLINE_M46_16796 '+JSON.stringify(row));
...
console.log('EARTHLINE_M46_16796_SUMMARY '+JSON.stringify(rows));
