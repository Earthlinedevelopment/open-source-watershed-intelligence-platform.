...
const OUT='artifacts/mantra47-16840-fine-gap-resolution-ab';
...
const variants=['control','grid24','grid24Opportunity'];
...
for(const variant of variants){
 ...
 await context.route('https://earthlinedevelopment.org/**',async route=>{
  ...
  if(variant!=='control'){
   for(const [a,b,label,count] of [[gridOld,gridNew,'grid',1],[bxOld,bxNew,'bx',1],[byOld,byNew,'by',1]]){if(body.split(a).length-1!==count)throw new Error(label+' occurrence mismatch '+(body.split(a).length-1));body=body.replace(a,b);}
   const gc=body.split(gapOld).length-1;if(gc<1)throw new Error('gap occurrence mismatch '+gc);body=body.split(gapOld).join(gapNew);
   const ac=body.split(auditOld).length-1;if(ac<1)throw new Error('audit occurrence mismatch '+ac);body=body.split(auditOld).join(auditNew);
  }
  if(variant==='grid24Opportunity'){
   if(body.split(metricOld).length-1!==1)throw new Error('metric occurrence mismatch');body=body.replace(metricOld,metricNew);
  }
  body=body.replace(capture,`window.__EARTHLINE_M47_16840={hy,candidates,chosen};${capture}`);
 ...
 try{await page.goto(BASE+`?m47_16840=${variant}_${Date.now()}`,{waitUntil:'domcontentloaded',timeout:45000});await page.waitForSelector('#searchInput',{timeout:30000});await page.evaluate(()=>{const i=document.getElementById('searchInput'),b=document.getElementById('runBtn');i.value='Arkansas';i.dispatchEvent(new Event('input',{bubbles:true}));i.dispatchEvent(new Event('change',{bubbles:true}));b.click();});try{await page.waitForFunction(()=>{const s=String(document.getElementById('earthlineVermontStatus16147')?.textContent||document.getElementById('earthlineTierNotice16173')?.textContent||'');return /screening published\./i.test(s)&&!!window.__EARTHLINE_M47_16840;},null,{timeout:65000,polling:100});}catch(_){timedOut=true;}await page.waitForTimeout(600);}catch(e){loadError=String(e);}
 const audit=loadError||timedOut?null:await page.evaluate(target=>{
  const {hy,candidates,chosen}=window.__EARTHLINE_M47_16840,[west,south,east,northBound]=[...hy.bounds];
  const ll=(gx,gy)=>({lng:west+(gx/hy.w)*(east-west),lat:northBound-(gy/hy.h)*(northBound-south)}),hav=(a,b)=>{const R=6371,rad=x=>x*Math.PI/180,dlat=rad(b.lat-a.lat),dlng=rad(b.lng-a.lng),q=Math.sin(dlat/2)**2+Math.cos(rad(a.lat))*Math.cos(rad(b.lat))*Math.sin(dlng/2)**2;return 2*R*Math.asin(Math.sqrt(q));},cp=c=>ll(Number(c.coverageGX16775??c.x),Number(c.coverageGY16775??c.y)),rings=[5,10,15,20,30,40],isNorth=c=>{const p=cp(c),d=((Math.atan2(p.lng-target.lng,p.lat-target.lat)*180/Math.PI)+360)%360;return d<=67.5||d>=292.5;};
  const fine=window.EARTHLINE_FINE_OPPORTUNITY_REFINEMENT_16781||null;
  return {candidates:candidates.length,chosen:chosen.length,candidateRings:rings.map(r=>({r,count:candidates.filter(c=>hav(target,cp(c))<=r).length})),selectedRings:rings.map(r=>({r,count:chosen.filter(c=>hav(target,cp(c))<=r).length})),northCandidateRings:rings.map(r=>({r,count:candidates.filter(c=>isNorth(c)&&hav(target,cp(c))<=r).length})),northSelectedRings:rings.map(r=>({r,count:chosen.filter(c=>isNorth(c)&&hav(target,cp(c))<=r).length})),fineGrid:fine?.grid,fineSelected:fine?.selected,fineAdded:fine?.added,fineFailed:fine?.failedTiles,fineTargetSelected:!!fine?.selected?.some(x=>Number(x.bx)===12&&Number(x.by)===11),fineTargetWestSelected:!!fine?.selected?.some(x=>Number(x.bx)===11&&Number(x.by)===11),fineTargetUnresolved:!!fine?.unresolvedAfter?.some(x=>Number(x.bx)===12&&Number(x.by)===11),pub:window.EARTHLINE_CORRIDOR_PUBLICATION_AUDIT_16167||null,disp:window.EARTHLINE_REGIONAL_DISPLAY_AUDIT_16040||window.EARTHLINE_REGIONAL_DISPLAY_AUDIT_16020||null,flow:window.EARTHLINE_LAND_VALIDITY_FLOW_AUDIT_16584||null,boundary:window.EARTHLINE_REGIONAL_JURISDICTION_BOUNDARY_AUDIT_16539||null,perf:window.EARTHLINE_REGIONAL_PERFORMANCE_16191||null};
 },target);
 const result={variant,loadError,timedOut,pageErrors,elapsedMs:Date.now()-started,audit};results.push(result);console.log('EARTHLINE_M47_16840 '+JSON.stringify(result));try{await page.screenshot({path:`${OUT}/${variant}.png`,fullPage:false});}catch(_){}await context.close();
}
