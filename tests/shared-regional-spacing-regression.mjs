...
const CASES=['Texas','Texas','New York','Vermont'];
...
try{await page.goto(BASE+'?earthline_shared_spacing_regression='+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});await page.waitForSelector('#searchInput',{timeout:30000});}catch(e){loadError=String(e);}
for(let run=0;run<CASES.length&&!loadError;run++){
  ...
  try{await page.waitForFunction(({q,prev})=>{const f=window.EARTHLINE_LAND_VALIDITY_FLOW_AUDIT_16584||null,e=window.EARTHLINE_LAST_LIVE_REGIONAL_ERROR_15970||null,token=(e&&e.runToken)||(f&&f.runToken)||null;if(!token||token===prev)return false;if(e&&e.runToken===token)return true;const m=typeof M!=='undefined'&&M?M:null,s=String(document.getElementById('earthlineVermontStatus16147')?.textContent||document.getElementById('earthlineTierNotice16173')?.textContent||''),loc=[m?.loc?.name,m?.loc?.fullName].filter(Boolean).join(' ');return loc.toLowerCase().includes(q.toLowerCase())&&/screening published\./i.test(s)&&!!window.EARTHLINE_REGIONAL_PERFORMANCE_16191;},{q:query,prev},{timeout:30000,polling:50});}catch(_){timedOut=true;}
  ...
  const r={run:run+1,query,patch,clickToTerminalMs:Date.now()-started,timedOut,coreMs:a?.perf?.totalMs??null,candidates:a?.generation?.candidates??null,eligible:a?.generation?.jurisdictionEligibleCandidates??null,chosen:a?.generation?.chosenBeforeTierGate??null,beforeClip:a?.boundary?.before?.swales??null,afterClip:a?.boundary?.after?.swales??null,removedByBoundary:a?.boundary?.removed?.swales??null,published:a?.pub?.generated??null,visible:a?.display?.swaleLines??null,gridW:a?.flow?.gridAudit?.grid?.w??null,gridH:a?.flow?.gridAudit?.grid?.h??null,unsafe:a?.flow?.unsafeSegments??null,lastError:a?.lastError||null,pageErrors:pageErrors.slice(),status:a?.status||''};
  results.push(r);console.log('EARTHLINE_SHARED_SPACING_REGRESSION '+JSON.stringify(r));
}
await browser.close();
writeFileSync('shared-regional-spacing-regression.json',JSON.stringify({loadError,patch,results},null,2));
const bad=loadError||patch.primaryMatches!==1||patch.secondaryMatches!==1||results.length!==CASES.length||results.some(r=>r.timedOut||r.lastError||r.pageErrors.length||Number(r.clickToTerminalMs)>15000||Number(r.coreMs)>15000||Number(r.gridW)!==96||Number(r.gridH)!==96||Number(r.unsafe)!==0||!/screening published\./i.test(r.status)||(Number(r.published||0)>0&&Number(r.visible||0)<=0)||(r.query==='Texas'&&(Number(r.removedByBoundary)!==0||Number(r.published)<40)));
