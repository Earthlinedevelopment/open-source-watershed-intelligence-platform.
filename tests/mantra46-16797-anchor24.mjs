...
const OUT='artifacts/mantra46-16797-anchor24';
...
for(const query of STATES){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    if(body.split(oldBx).length-1!==1||body.split(oldBy).length-1!==1)throw new Error('16786 anchor owner not uniquely found');
    body=body.replace(oldBx,newBx).replace(oldBy,newBy);

    const cap="window.EARTHLINE_SWALE_GENERATION_AUDIT_16167=generationAudit;";
    const neu="window.__EARTHLINE_M46_16797_CHAIN={hy,candidates,chosen};"+cap;
