...
const OUT='artifacts/mantra46-16808-cluster-spread-ab';
...
async function runOne(query,variant){
 ...
 await context.route('https://earthlinedevelopment.org/**',async route=>{
  ...
  if(variant){
   ...
   const sortOld='missing16783.sort((a16783,b16783)=>b16783.nearest-a16783.nearest||b16783.count-a16783.count||(Number(b16783.row.candidate.score)||0)-(Number(a16783.row.candidate.score)||0));';
   const sortNew=`{
     const map16808=new Map(missing16783.map(m16808=>[m16808.row.point.cell,m16808])),seen16808=new Set();
     for(const m16808 of missing16783){
       const k016808=m16808.row.point.cell;if(seen16808.has(k016808))continue;
       const q16808=[k016808],component16808=[];seen16808.add(k016808);
       while(q16808.length){
         const k16808=q16808.shift(),cur16808=map16808.get(k16808);if(!cur16808)continue;component16808.push(cur16808);
         const parts16808=k16808.split(',').map(Number);
         for(let oy16808=-1;oy16808<=1;oy16808++)for(let ox16808=-1;ox16808<=1;ox16808++){
           if(!ox16808&&!oy16808)continue;const nk16808=(parts16808[0]+ox16808)+','+(parts16808[1]+oy16808);
           if(map16808.has(nk16808)&&!seen16808.has(nk16808)){seen16808.add(nk16808);q16808.push(nk16808);}
         }
       }
       for(const c16808 of component16808)c16808.cluster16808=component16808.length;
     }
     missing16783.sort((a16783,b16783)=>(Number(b16783.cluster16808)||0)-(Number(a16783.cluster16808)||0)||b16783.nearest-a16783.nearest||b16783.count-a16783.count||(Number(b16783.row.candidate.score)||0)-(Number(a16783.row.candidate.score)||0));
   }`;
   if(body.split(sortOld).length-1!==1)throw new Error('missing-cell sort owner mismatch');body=body.replace(sortOld,sortNew);
  }
  ...
  if(body.split(call).length-1!==1)throw new Error('main call not found');body=body.replace(call,'window.__EARTHLINE_M46_16808_HY=hy;'+call);
 ...
 const audit=loadError||timedOut?null:await page.evaluate(({query,target})=>{
  ...
  return{coreMs:perf?.totalMs??null,generated:pub?.generated??null,visible:display?.swaleLines??pub?.overlaySwaleLines??null,unsafe:flow?.unsafeSegments??null,outside:boundary?.outsideAfterClip??null,capacity:score?.capacity16755??null,targetNearest,spread:{candidateCells:spread?.candidateCells,chosenCount:spread?.chosenCount,finalCells:spread?.finalCells,swaps:spread?.swaps,stop:spread?.selectionStopReason},qualified:qualified.length,covered:qualified.length-unserved.length,unservedKeys:unserved.map(b=>b.bx+','+b.by)};
 },{query,target:TARGET});
 ...
 const row={query,variant:variant?'cluster-first-final-spread':'control',loadError,timedOut,pageErrors,elapsedMs:Date.now()-started,audit};rows.push(row);console.log('EARTHLINE_M46_16808 '+JSON.stringify(row));try{await page.screenshot({path:OUT+'/'+query.toLowerCase()+'-'+(variant?'cluster':'control')+'.png',fullPage:false});}catch(_){ }await context.close();
}
for(const q of STATES){await runOne(q,false);await runOne(q,true)}
await browser.close();writeFileSync(OUT+'/results.json',JSON.stringify(rows,null,2));console.log('EARTHLINE_M46_16808_SUMMARY '+JSON.stringify(rows));const bad=rows.filter(r=>!r.audit||r.loadError||r.timedOut||r.pageErrors.length||Number(r.audit.coreMs)>15000||Number(r.audit.generated)!==Number(r.audit.visible)||Number(r.audit.unsafe)!==0||Number(r.audit.outside?.swales||0)!==0);if(bad.length)process.exitCode=1;
