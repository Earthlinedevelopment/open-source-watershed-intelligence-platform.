...
const FACTORS=[1,.85,.7,.55,.4,.25];
const browser=await chromium.launch({headless:true});
const rows=[];
for(const factor of FACTORS){
  const page=await browser.newPage({viewport:{width:1800,height:1000}});
  let resPatch=0,scorePatch=0;
  await page.route('**/*',async route=>{
    if(route.request().resourceType()!=='document')return route.continue();
    const resp=await route.fetch();let body=await resp.text();
    const a=`const d16702=await loadDEM(tile16702.b,112,112,6000,'scale refinement '+tile16702.id)`;
    const ar=`const localRes16710=tile16702.id==='coast-1'?112:96,d16702=await loadDEM(tile16702.b,localRes16710,localRes16710,6000,'scale refinement '+tile16702.id)`;
    resPatch=body.split(a).length-1;if(resPatch!==1)throw new Error('resolution anchor '+resPatch);body=body.replace(a,ar);
    const b=`score:Math.max(0,Math.min(1,Number(r.f.properties?.score||0)/100)),confidence:'preferred',minLinePx16632:4,refined16702:true`;
    const br=`score:Math.max(0,Math.min(1,Number(r.f.properties?.score||0)/100))*${factor},confidence:'preferred',minLinePx16632:4,refined16702:true`;
    scorePatch=body.split(b).length-1;if(scorePatch!==1)throw new Error('score anchor '+scorePatch);body=body.replace(b,br);
    return route.fulfill({response:resp,body});
  });
  await page.goto(URL+'?tx_refined_weight='+factor+'&t='+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
  ...
  for(let repeat=1;repeat<=2;repeat++){
    ...
    const snap=await page.evaluate(()=>{
      ...
      return {swales:sw.length,visible:d?.swaleLines??null,published:g?.publishedFeatures??null,totalMs:p?.totalMs??null,candidates:g?.candidates??null,eligible:g?.jurisdictionEligibleCandidates??null,selection:window.EARTHLINE_REFINED_SELECTION_16702||null,unsafe:flow?.unsafeSegments??null,outside:b?.outsideAfterClip??null,lastError:window.EARTHLINE_LAST_LIVE_REGIONAL_ERROR_15970||null,zones:{panhandle:count((x,y)=>x>-103.1&&x<-100&&y>35&&y<36.6),upperGulf:count((x,y)=>x>-96.5&&x<-93.45&&y>28.8&&y<31.2),midGulf:count((x,y)=>x>-99.3&&x<-96&&y>27.4&&y<30.2),lowerGulf:count((x,y)=>x>-99.5&&x<-97&&y>25.7&&y<28.2),east:count((x,y)=>x>-96&&x<-93.45&&y>30.5&&y<34.3)}};
    });
    const row={factor,repeat,elapsedMs:Date.now()-started,timedOut,resPatch,scorePatch,snap};rows.push(row);console.log('EARTHLINE_TX_REFINED_WEIGHT '+JSON.stringify(row));
  }
  await page.close();
}
console.log('EARTHLINE_TX_REFINED_WEIGHT_SUMMARY '+JSON.stringify(rows));
await browser.close();