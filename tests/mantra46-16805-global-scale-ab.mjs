...
const OUT='artifacts/mantra46-16805-global-scale-ab';
...
async function runOne(query,variant){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    if(variant){
      ...
      const capNew='const regionalCapacity16755=focusMode?80:Math.min(180,Math.max(80,coverageGroups16736.size*5));';
      if(body.split(capOld).length-1!==1)throw new Error('capacity owner mismatch');body=body.replace(capOld,capNew);
      const gridOld='const nx16783=12,ny16783=12;';
      const gridNew='const nx16783=24,ny16783=24;';
      if(body.split(gridOld).length-1!==1)throw new Error('spread grid owner mismatch');body=body.replace(gridOld,gridNew);
    ...
    if(body.split(call).length-1!==1)throw new Error('main call not found');body=body.replace(call,"window.__EARTHLINE_M46_16805_HY=hy;"+call);
  ...
  const audit=loadError||timedOut?null:await page.evaluate(({query,target})=>{
    ...
    return{coreMs:perf?.totalMs??null,generated:pub?.generated??null,visible:display?.swaleLines??pub?.overlaySwaleLines??null,unsafe:flow?.unsafeSegments??null,outside:boundary?.outsideAfterClip??null,declaredCapacity:score?.capacity16755??spatial?.capacity??null,candidateBins:spatial?.candidateBins??null,targetNearest,spread:{candidateCells:spread?.candidateCells,chosenCount:spread?.chosenCount,initialCells:spread?.initialCells,finalCells:spread?.finalCells,targetCells:spread?.targetCells,stop:spread?.selectionStopReason,swaps:spread?.swaps},qualified:qualified.length,covered:qualified.length-unserved.length,unservedKeys:unserved.map(b=>b.bx+','+b.by)};
  },{query,target:TARGET});
  ...
  const row={query,variant:variant?'24x24-plus-capacity':'control',loadError,timedOut,pageErrors,elapsedMs:Date.now()-started,audit};rows.push(row);console.log('EARTHLINE_M46_16805 '+JSON.stringify(row));try{await page.screenshot({path:OUT+'/'+query.toLowerCase()+'-'+(variant?'global':'control')+'.png',fullPage:false});}catch(_){ }await context.close();
}
for(const q of STATES){await runOne(q,false);await runOne(q,true)}
await browser.close();writeFileSync(OUT+'/results.json',JSON.stringify(rows,null,2));console.log('EARTHLINE_M46_16805_SUMMARY '+JSON.stringify(rows));const bad=rows.filter(r=>!r.audit||r.loadError||r.timedOut||r.pageErrors.length||Number(r.audit.coreMs)>15000||Number(r.audit.generated)!==Number(r.audit.visible)||Number(r.audit.unsafe)!==0||Number(r.audit.outside?.swales||0)!==0);if(bad.length)process.exitCode=1;
