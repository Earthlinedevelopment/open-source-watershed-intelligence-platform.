    ...
    if(!focusMode&&/texas/i.test(String(q||''))){
      ...
      try{
        ...
        const tileResults16672=await Promise.all(tiles16672.map(async tile16672=>{
          ...
          if(elevs16672.length){for(const frac16672 of [.05,.12,.22,.34]){const e16672=elevs16672[Math.min(elevs16672.length-1,Math.floor((elevs16672.length-1)*frac16672))],k16672=Math.round(e16672*2)/2;if(!levelSeen16672.has(k16672)){levelSeen16672.add(k16672);levels16672.push(k16672);}}}
