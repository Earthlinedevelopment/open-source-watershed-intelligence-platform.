...
await page.route('**/*',async route=>{
 ...
 const repl=`    const gapAttempts16621=[];
    ...
    let gapAdded16621=0;
    const shortAttemptEvaluated16629=new WeakSet();
    function rawCanReachFour16629(attempt16629){
      const coords16629=attempt16629&&attempt16629[0],center16629=Number(attempt16629&&attempt16629[1]);
      if(!Array.isArray(coords16629)||!Number.isFinite(center16629))return false;
      const half16629=Math.max(10,Math.min(52,Math.floor(coords16629.length*.18))),start16629=Math.max(0,center16629-half16629),end16629=Math.min(coords16629.length,center16629+half16629+1),raw16629=coords16629.slice(start16629,end16629);
      return raw16629.length>=10&&lineLengthPixels(hy,raw16629)>=4;
    }
    if(coarseGapPass16621&&gapAttempts16621.length){
      ...
      for(const attempt16621 of gapAttempts16621){
        ...
        if(!rawCanReachFour16629(attempt16621))continue;
        shortAttemptEvaluated16629.add(attempt16621);
        gapEvaluated16621++;
    ...
    if(coarseGapPass16621&&hy&&hy.validityMask16584){
      for(const attempt16626 of gapAttempts16621){
        ...
        if(shortAttemptEvaluated16629.has(attempt16626)||!rawCanReachFour16629(attempt16626))continue;
        shortAttemptEvaluated16629.add(attempt16626);
