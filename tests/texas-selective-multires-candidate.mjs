...
await page.route('**/*',async route=>{
 ...
 const ar=`const tilesMulti16712=tiles16702.flatMap((t16712,i16712)=>i16712===tiles16702.length-1?[Object.assign({},t16712,{baseId16712:t16712.id,res16712:112,id:t16712.id+'-r112'}),Object.assign({},t16712,{baseId16712:t16712.id,res16712:96,id:t16712.id+'-r96'})]:[Object.assign({},t16712,{baseId16712:t16712.id,res16712:112,id:t16712.id+'-r112'})]);\n        const tileResults16702=await Promise.all(tilesMulti16712.map(async tile16702=>{`;
 ...
 const br=`const d16702=await loadDEM(tile16702.b,tile16702.res16712,tile16702.res16712,6000,'scale refinement '+tile16702.id)`;
 ...
 const cr=`tile16702:(tile16702.baseId16712||tile16702.id)`;
 tilePatch=body.split(c).length-1;if(tilePatch<1)throw new Error('tile anchor '+tilePatch);body=body.split(c).join(cr);
 return route.fulfill({response:resp,body});
});
await page.goto(URL+'?tx_selective_multi='+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
...
for(let repeat=1;repeat<=3;repeat++){
 ...
 const row={repeat,elapsedMs:Date.now()-started,timedOut,expandPatch,resPatch,tilePatch,snap};rows.push(row);console.log('EARTHLINE_TX_SELECTIVE_MULTI '+JSON.stringify(row));
}
console.log('EARTHLINE_TX_SELECTIVE_MULTI_SUMMARY '+JSON.stringify(rows));
await browser.close();
