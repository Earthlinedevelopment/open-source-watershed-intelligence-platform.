...
await page.route('**/*',async route=>{
 ...
 const repl=`    const gapAttempts16621=[];
    ...
    if(coarseGapPass16621&&gapAttempts16621.length){
      ...
      const rankedAttempts16623=gapAttempts16621.map(attempt16621=>{
        const p16621=attempt16621[0][attempt16621[1]],g16621=llGrid(hy,p16621);
        if(!g16621||!Number.isFinite(g16621.x)||!Number.isFinite(g16621.y))return null;
        let nearest16623=Infinity;
        for(const q16621 of covered16621)nearest16623=Math.min(nearest16623,Math.hypot(Number(q16621.x)-Number(g16621.x),Number(q16621.y)-Number(g16621.y)));
        return {attempt16621,g16621,nearest16623};
      }).filter(Boolean).filter(x16623=>x16623.nearest16623>=8).sort((a16623,b16623)=>b16623.nearest16623-a16623.nearest16623).slice(0,48);
      for(const item16623 of rankedAttempts16623){
        if(gapAdded16621>=12)break;
        const attempt16621=item16623.attempt16621,g16621=item16623.g16621;
      ...
      window.EARTHLINE_GAP_FALLBACK_SHORTLISTED_16623=rankedAttempts16623.length;
    }
