...
const OUT='artifacts/mantra47-16826-fine-grid-ab';
const MODES=[18,24,36,48];
...
for(const N of MODES){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    if(body.split(sx).length-1!==1||body.split(sy).length-1!==1||body.split(sp).length-1!==1||body.split(capture).length-1!==1) throw new Error('16778 owner/capture occurrence mismatch');
    if(N!==18){
    ...
    body=body.replace(capture,"window.__EARTHLINE_M47_16826_CHAIN={hy,candidates,chosen,map:(typeof map!=='undefined'?map:null)};"+capture);
  ...
  try{
    await page.goto(BASE+`?m47_16826=${N}_${Date.now()}`,{waitUntil:'domcontentloaded',timeout:45000});
    ...
    try{
      await page.waitForFunction(()=>{
        ...
        return /screening published\./i.test(s)&&!!window.__EARTHLINE_M47_16826_CHAIN;
  ...
  const audit=loadError||timedOut?null:await page.evaluate(({T,N})=>{
    const {hy,candidates,chosen}=window.__EARTHLINE_M47_16826_CHAIN;
    ...
    const fineKey=c=>{const gx=Number.isFinite(Number(c?.coverageGX16775))?Number(c.coverageGX16775):Number(c?.x),gy=Number.isFinite(Number(c?.coverageGY16775))?Number(c.coverageGY16775):Number(c?.y);if(!Number.isFinite(gx)||!Number.isFinite(gy))return null;const fx=Math.max(0,Math.min(N-1,Math.floor(gx*N/Math.max(1,hy.w)))),fy=Math.max(0,Math.min(N-1,Math.floor(gy*N/Math.max(1,hy.h))));return `${fx},${fy}`;};
    const parentKey=c=>{const k=fineKey(c);if(!k)return null;const [fx,fy]=k.split(',').map(Number);return `${Math.floor(fx*6/N)},${Math.floor(fy*6/N)}`;};
    const rows=arr=>(arr||[]).map((c,i)=>({i,c,d:minDist(c)})).filter(r=>Number.isFinite(r.d)).sort((a,b)=>a.d-b.d);
    const summary=rs=>{const counts={};for(const r of [5,10,20,40,80,120])counts[r]=rs.filter(x=>x.d<=r).length;return {total:rs.length,counts,nearest:rs.slice(0,15).map(r=>({i:r.i,d:Number(r.d.toFixed(3)),score:Number(r.c?.score??NaN),x:Number(r.c?.x??NaN),y:Number(r.c?.y??NaN),fineCell:fineKey(r.c),parent:parentKey(r.c)}))};};
    ...
    return {
      N,
      candidates:summary(cRows),chosen:summary(sRows),
      ...
      fine:fine?{candidateCells:fine.candidateCells??null,chosenCount:fine.chosenCount??null,initialChosenCells:fine.initialChosenCells??null,finalChosenCells:fine.finalChosenCells??null,initialCoverage:fine.initialCoverage??null,finalCoverage:fine.finalCoverage??null,missingInitial:fine.missingInitial??null,swaps:fine.swaps??null,targetCells:fine.targetCells??null}:null,
      spread:spread?{initialCells:spread.initialCells??null,finalCells:spread.finalCells??null,targetCells:spread.targetCells??null,attainableTargetCells:spread.attainableTargetCells??null,stop:spread.selectionStopReason??null,swaps:spread.swaps??null}:null
    };
  },{T:TARGET,N});

  const result={N,loadError,timedOut,pageErrors,elapsedMs:Date.now()-started,audit};
  results.push(result);
  writeFileSync(`${OUT}/grid-${N}.json`,JSON.stringify(result,null,2));
  try{await page.screenshot({path:`${OUT}/arkansas-grid-${N}.png`,fullPage:false});}catch(_){}
  console.log('EARTHLINE_M47_16826 '+JSON.stringify(result));
  await context.close();
}
