...
const out='artifacts/mantra46-16817-fine-min6-16809-gate';
...
replaceOne("const OUT='artifacts/mantra46-16809-opportunity-gap-swap-ab';","const OUT='artifacts/mantra46-16817-fine-min6-16809-gate';",'out');
const variantAnchor="if(variant){\n   const capOld=";
if(s.split(variantAnchor).length-1!==1)throw new Error('variant anchor mismatch');
s=s.replace(variantAnchor,`if(variant){\n   const fineBudgetOld="const refinementTileBudget16782=Math.min(16,Math.max(8,Math.ceil(gapsBefore16780.length/3)));";\n   if(body.split(fineBudgetOld).length-1!==1)throw new Error('fine budget owner mismatch');\n   body=body.replace(fineBudgetOld,"const refinementTileBudget16782=Math.min(16,Math.max(6,Math.ceil(gapsBefore16780.length/3)));" );\n   const capOld=`);
replaceOne(
 "oppSwap=window.EARTHLINE_OPPORTUNITY_GAP_SWAP_16809||null;",
 "oppSwap=window.EARTHLINE_OPPORTUNITY_GAP_SWAP_16809||null,fine16817=window.EARTHLINE_FINE_OPPORTUNITY_REFINEMENT_16781||null;",
 'audit refs'
);
replaceOne(
 "coreMs:perf?.totalMs??null,",
 "coreMs:perf?.totalMs??null,fine16817:{elapsedMs:fine16817?.elapsedMs??0,selected:fine16817?.selected?.length??0,added:fine16817?.added??0,failed:fine16817?.failedTiles?.length??0,unresolved:fine16817?.unresolvedAfter?.length??0},",
 'audit return'
);
replaceOne(
