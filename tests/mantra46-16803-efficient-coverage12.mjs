...
const OUT='artifacts/mantra46-16803-efficient-coverage12';
...
for(const query of STATES){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    const selectOld="const selected16731=[];for(const gap16731 of gaps16731){if(selected16731.length>=18)break;selected16731.push(gap16731);}";
    const selectNew="const selected16731=[];for(const gap16731 of gaps16731){if(selected16731.length>=12)break;selected16731.push(gap16731);}";
    if(body.split(selectOld).length-1!==1)throw new Error('coverage tile budget owner not found');
    body=body.replace(selectOld,selectNew);

    const firstDemOld="loadDEM(tile16731.b,48,48,8000,'coverage-gap refinement '+tile16731.id)";
    const firstDemNew="loadDEM(tile16731.b,32,32,8000,'coverage-gap refinement '+tile16731.id)";
    if(body.split(firstDemOld).length-1!==1)throw new Error('coverage refinement DEM owner not found');
    body=body.replace(firstDemOld,firstDemNew);

    const lateBinsOld="const lateBins16741=unresolved16731.slice(0,4),lateStart16741=performance.now();";
    const lateBinsNew="const lateBins16741=unresolved16731.slice(0,2),lateStart16741=performance.now();";
    if(body.split(lateBinsOld).length-1!==1)throw new Error('late gap budget owner not found');
    body=body.replace(lateBinsOld,lateBinsNew);

    const lateDemOld="loadDEM(tile16741.b,96,96,8000,'late coverage-gap verification '+tile16741.id)";
    const lateDemNew="loadDEM(tile16741.b,48,48,8000,'late coverage-gap verification '+tile16741.id)";
    if(body.split(lateDemOld).length-1!==1)throw new Error('late gap DEM owner not found');
    body=body.replace(lateDemOld,lateDemNew);
    ...
    body=body.replace(genAudit,"window.__EARTHLINE_M46_16803_CHAIN={hy,candidates,chosen};"+genAudit);
  ...
  try{
    await page.goto(BASE+'?m46_16803='+encodeURIComponent(query)+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
    ...
    try{
      await page.waitForFunction(q=>{
        ...
        return /screening published\./i.test(s)&&!!window.__EARTHLINE_M46_16803_CHAIN&&root&&String(root.query||'').toLowerCase().includes(q.toLowerCase());
  ...
  const audit=loadError||timedOut?null:await page.evaluate(()=>{
    const {hy,chosen}=window.__EARTHLINE_M46_16803_CHAIN;
    ...
    return {
      ...
      gapElapsedMs:gapRefine?.elapsedMs??null,\n      lateGapSelected:lateGap?.selected?.length??null,\n      lateGapAdded:lateGap?.added??null,\n      lateGapElapsedMs:lateGap?.elapsedMs??null
    };
  });
  ...
  rows.push(row);
  console.log('EARTHLINE_M46_16803 '+JSON.stringify(row));
...
console.log('EARTHLINE_M46_16803_SUMMARY '+JSON.stringify(rows));
