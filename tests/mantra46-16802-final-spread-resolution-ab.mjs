...
const OUT='artifacts/mantra46-16802-final-spread-resolution-ab';
...
async function runOne(query,variant){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    body=body.replace(call,"window.__EARTHLINE_M46_16802_HY=hy;"+call);
    const cap="window.EARTHLINE_SWALE_GENERATION_AUDIT_16167=generationAudit;";
    if(body.split(cap).length-1!==1)throw new Error('generation owner not found');
    body=body.replace(cap,"if(hy===window.__EARTHLINE_M46_16802_HY)window.__EARTHLINE_M46_16802_CHAIN={candidates:[...candidates],chosen:[...chosen]};"+cap);
  ...
  try{
    await page.goto(BASE+'?m46_16802='+encodeURIComponent(query)+'_'+(variant?'24':'12')+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
    ...
    try{await page.waitForFunction(q=>{const s=String(document.getElementById('earthlineVermontStatus16147')?.textContent||document.getElementById('earthlineTierNotice16173')?.textContent||'');const root=window.EARTHLINE_STANDARD_ROOT_CAUSE_AUDIT_16784||null;return /screening published\./i.test(s)&&!!window.__EARTHLINE_M46_16802_CHAIN&&root&&String(root.query||'').toLowerCase().includes(q.toLowerCase());},query,{timeout:65000,polling:100});}catch(_){timedOut=true;}
    await page.waitForTimeout(600);
  }catch(e){loadError=String(e);}

  const audit=loadError||timedOut?null:await page.evaluate(({query,target})=>{
    const hy=window.__EARTHLINE_M46_16802_HY,chain=window.__EARTHLINE_M46_16802_CHAIN;
    const visual=window.EARTHLINE_REGIONAL_VISUAL_DATA_16020||null,swales=visual?.swales?.features||[];
    ...
    return {coreMs:perf?.totalMs??null,generated:pub?.generated??null,visible:display?.swaleLines??pub?.overlaySwaleLines??null,unsafe:flow?.unsafeSegments??null,outside:boundary?.outsideAfterClip??null,candidates:chain?.candidates?.length||0,chosen:chain?.chosen?.length||0,targetNearest,spread:{candidateCells:spread?.candidateCells,chosenCount:spread?.chosenCount,initialCells:spread?.initialCells,finalCells:spread?.finalCells,targetCells:spread?.targetCells,stop:spread?.selectionStopReason,swaps:spread?.swaps},qualified:qualified.length,covered:qualified.length-unserved.length,unservedKeys:unserved.map(b=>b.bx+','+b.by)};
  },{query,target:TARGET});
  ...
  const row={query,variant:variant?'24x24-final-spread':'12x12-control',loadError,timedOut,pageErrors,elapsedMs:Date.now()-started,audit};rows.push(row);console.log('EARTHLINE_M46_16802 '+JSON.stringify(row));
  try{await page.screenshot({path:OUT+'/'+query.toLowerCase()+'-'+(variant?'24':'12')+'.png',fullPage:false});}catch(_){ }
  await context.close();
}

for(const q of STATES){await runOne(q,false);await runOne(q,true)}
await browser.close();writeFileSync(OUT+'/results.json',JSON.stringify(rows,null,2));console.log('EARTHLINE_M46_16802_SUMMARY '+JSON.stringify(rows));
