...
for(const query of STATES){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    const old="window.EARTHLINE_SWALE_GENERATION_AUDIT_16167=generationAudit;";
    const neu="window.__EARTHLINE_M46_CHAIN_16795={hy,candidates,chosen};window.EARTHLINE_SWALE_GENERATION_AUDIT_16167=generationAudit;";
    const count=body.split(old).length-1;
    if(count!==1)throw new Error('internal makeSwales capture owner count='+count);
  ...
  const audit=loadError||timedOut?null:await page.evaluate(()=>{
    ...
    return {
      ...
      selectedCovered:chosenCover.size,
      noCandidateKeys:q.filter(k=>!candidateCover.has(k)),
      selectionMissKeys:q.filter(k=>candidateCover.has(k)&&!chosenCover.has(k))
    };
  });
...
console.log('EARTHLINE_M46_CHAIN2_SUMMARY '+JSON.stringify(rows));
