...
for(const query of STATES){
  ...
  await context.route('https://earthlinedevelopment.org/**',async route=>{
    ...
    const neu="try{window.__EARTHLINE_M46_CHAIN_16795={hy:hy,candidates:candidates,chosen:chosen};earthlineStandardAudit16784({tier:focusMode?'focus':'regional',runToken,query:q,hy,stage:'core-published'});}catch(_){ }";
    const count=body.split(old).length-1;
    if(count!==1)throw new Error('chain insertion owner count='+count);
  ...
  try{
    await page.goto(BASE+'?m46_chain='+encodeURIComponent(query)+'_'+Date.now(),{waitUntil:'domcontentloaded',timeout:45000});
    ...
    try{
      await page.waitForFunction(q=>{
        ...
        return /screening published\./i.test(s)&&!!window.__EARTHLINE_M46_CHAIN_16795&&root&&String(root.query||'').toLowerCase().includes(q.toLowerCase());
  ...
  const audit=loadError||timedOut?null:await page.evaluate(()=>{
    const {hy,candidates,chosen}=window.__EARTHLINE_M46_CHAIN_16795;
    const nx=24,ny=24;
    ...
    const footprint=(c)=>{
      ...
      if(seg&&seg.length){
        ...
        for(const ll of seg){
          ...
          if(g&&Number.isFinite(Number(g.x))&&Number.isFinite(Number(g.y)))gridPts.push([Number(g.x),Number(g.y)]);
        }
        for(let i=0;i<gridPts.length;i++){
          const [x,y]=gridPts[i];
          out.add(binAtGrid(x,y).bx+','+binAtGrid(x,y).by);
          if(i===0)continue;
          const [ax,ay]=gridPts[i-1],dx=x-ax,dy=y-ay,steps=Math.max(1,Math.ceil(Math.hypot(dx,dy)*2));
          for(let s=1;s<steps;s++){
            const t=s/steps,b=binAtGrid(ax+dx*t,ay+dy*t);out.add(b.bx+','+b.by);
          }
        }
  ...
  rows.push(row);
  console.log('EARTHLINE_M46_CHAIN '+JSON.stringify(row));
  await context.close();
}
await browser.close();
writeFileSync(OUT+'/results.json',JSON.stringify(rows,null,2));
console.log('EARTHLINE_M46_CHAIN_SUMMARY '+JSON.stringify(rows.map(r=>({query:r.query,...(r.audit||{})}))));
