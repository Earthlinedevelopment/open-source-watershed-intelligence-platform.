...
for(const stateName of STATES){
  ...
  try{
    await page.goto(URL+'?m4516785='+encodeURIComponent(stateName)+'-'+Date.now(),{waitUntil:'domcontentloaded',timeout:60000});
    ...
    await page.waitForFunction(expected=>{
      ...
      return identityMatches && (!!err || (spatial?.build==='EARTHLINE 16785'&&generated>0&&visible===generated));
  ...
  if(snap.build!=='EARTHLINE 16785')failures.push('build '+snap.build);
...
fs.writeFileSync(process.env.OUT||'machine16785.json',JSON.stringify(out,null,2));
