...
await page.goto(URL+'?validate16714='+Date.now(),{waitUntil:'domcontentloaded',timeout:60000});
...
if(!(Number(tx?.snap?.regions?.upperGulf||0)+Number(tx?.snap?.regions?.lowerGulf||0)>=18))failures.push('Texas: Gulf occupancy still sparse');

const out={at:new Date().toISOString(),url:URL,rows,failures,pass:failures.length===0};
fs.writeFileSync(process.env.OUT||'lab/16714-validation.json',JSON.stringify(out,null,2));
