...
await page.goto(URL+'?validate16713='+Date.now(),{waitUntil:'domcontentloaded',timeout:60000});
...
for(const stateName of STATES){
  ...
  await page.evaluate(q=>{
    ...
    window.EARTHLINE_REFINED_COVERAGE_SELECTION_16713=null;
...
if(!(Number(cov.reserved||0)>=16))failures.push('Texas: refined reserve too low '+cov.reserved);
...
if(!(Number(tx?.snap?.regions?.upperGulf||0)+Number(tx?.snap?.regions?.lowerGulf||0)>=14))failures.push('Texas: Gulf occupancy still sparse');

const out={at:new Date().toISOString(),url:URL,rows,failures,pass:failures.length===0};
fs.writeFileSync(process.env.OUT||'lab/16713-validation.json',JSON.stringify(out,null,2));
