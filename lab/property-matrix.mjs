...
function snap(){
  ...
  return {
    ...
    locName:String(m&&m.loc&&m.loc.name||''),locFullName:String(m&&m.loc&&m.loc.fullName||''),
    ...
    audit:a,
    publication:p,
    vector:m&&m.vectorNoBuildCoverage||null,
    ...
    debugVisible:debug,
    status:String(document.getElementById('earthlineVermontStatus16147')?.textContent||document.getElementById('earthlineTierNotice16173')?.textContent||'').trim().slice(0,800)
  };
}

async function regional(frame,q){
  const before=await frame.evaluate(snap);const started=Date.now();
  await frame.evaluate(query=>{const i=document.getElementById('searchInput'),b=document.getElementById('runBtn');i.value=query;i.dispatchEvent(new Event('input',{bubbles:true}));i.dispatchEvent(new Event('change',{bubbles:true}));b.click();},q);
  let timeout=false;
  try{await frame.waitForFunction(({q,g})=>{const n=s=>String(s||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();const m=typeof M!=='undefined'&&M,r=window.earthlineRegional15778||{},d=window.EARTHLINE_DISPLAYED_RUN_16151||window.EARTHLINE_DISPLAYED_RUN_16147||{};const text=n([m&&m.loc&&m.loc.name,m&&m.loc&&m.loc.fullName,d.name,d.label,d.location].join(' '));return Number(m&&m.searchGen||0)>=Number(g||0)&&n(q).split(' ').every(w=>text.includes(w))&&r.active!==true&&document.getElementById('runBtn')?.getAttribute('aria-busy')!=='true';},{q,g:before.searchGen},{timeout:WAIT_LIMIT_MS,polling:200});}catch(_){timeout=true;}
  const s=await frame.evaluate(snap);const elapsedMs=Date.now()-started;
...
async function property(frame){
  ...
  const pre=await frame.evaluate(snap);const errorStart=consoleErrors.length;const started=Date.now();
  ...
  const elapsedMs=Date.now()-started;const post=await frame.evaluate(snap);const a=post.audit||{};
  ...
  if(!timeout&&!post.debugVisible&&!staleRegional&&perf&&published)classification='PASS';
  else if(!timeout&&!post.debugVisible&&!staleRegional&&perf&&safelyBlocked)classification='SAFE_BLOCK';
  return {classification,pass:classification==='PASS',safeBlock:classification==='SAFE_BLOCK',timeout,elapsedMs,performancePass:perf,published,safelyBlocked,staleRegional,pre,post,errors:consoleErrors.slice(errorStart,errorStart+20)};
}
...
for(const q of TESTS){
  ...
  try{
    const reg=await regional(frame,q);
    row.regional=reg;
    if(!reg.pass){row.classification='REGIONAL_FAIL';row.pass=false;}
    else{const prop=await property(frame);row.property=prop;row.classification=prop.classification;row.pass=prop.pass;}
  }catch(e){row.classification='HARNESS_FAIL';row.pass=false;row.error=String(e);}
  rows.push(row);
  console.log(`${AREA} ${q}: ${row.classification}`);
...
const report={generatedAt:new Date().toISOString(),url:URL,labBuild:16600,acceptedParent:16584,protocol:{hardCeilingMs:HARD_CEILING_MS,realPropertyControl:true,acceptedParentUnchanged:true,sharedCoreOnly:true},summary,rows};
