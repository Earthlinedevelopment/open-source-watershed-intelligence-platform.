...
for(const stateName of STATES){
  ...
  await page.evaluate(q=>{
    ...
    window.EARTHLINE_SCALE_REFINED_INPUT_16702=null;window.EARTHLINE_CORRIDOR_PUBLICATION_AUDIT_16167=null;window.EARTHLINE_REGIONAL_DISPLAY_AUDIT_16040=null;window.EARTHLINE_SWALE_GENERATION_AUDIT_16167=null;window.EARTHLINE_LAND_VALIDITY_GRID_AUDIT_16584=null;window.EARTHLINE_LAND_VALIDITY_16584=null;window.EARTHLINE_REGIONAL_CAMERA_AUDIT_16147=null;
  ...
  const snap=await page.evaluate(q=>{
    ...
    const g=window.EARTHLINE_SWALE_GENERATION_AUDIT_16167||null,pub=window.EARTHLINE_CORRIDOR_PUBLICATION_AUDIT_16167||null,d=window.EARTHLINE_REGIONAL_DISPLAY_AUDIT_16040||null,p=window.EARTHLINE_REGIONAL_PERFORMANCE_16191||null,b=window.EARTHLINE_REGIONAL_JURISDICTION_BOUNDARY_AUDIT_16539||null,flow=window.EARTHLINE_LAND_VALIDITY_FLOW_AUDIT_16584||null,grid=window.EARTHLINE_LAND_VALIDITY_GRID_AUDIT_16584||null,land=window.EARTHLINE_LAND_VALIDITY_16584||null,camera=window.EARTHLINE_REGIONAL_CAMERA_AUDIT_16147||null,extent=window.EARTHLINE_REGIONAL_ANALYSIS_EXTENT_16712||null;
    return {trigger:window.EARTHLINE_SCALE_REFINEMENT_TRIGGER_16705||null,input:window.EARTHLINE_SCALE_REFINED_INPUT_16702||null,swales:sw.length,visible:d?.swaleLines??null,published:pub?.published??pub?.generated??sw.length,generationPublished:g?.publishedFeatures??null,candidates:g?.candidates??null,eligible:g?.jurisdictionEligibleCandidates??null,totalMs:p?.totalMs??null,unsafe:flow?.unsafeSegments??null,outside:b?.outsideAfterClip??null,regions,generation:g,publication:pub,grid,land,camera,extent,lastError:window.EARTHLINE_LAST_LIVE_REGIONAL_ERROR_15970||null};
  },stateName);
  rows.push({state:stateName,elapsedMs:Date.now()-started,timedOut,snap});
  fs.writeFileSync(process.env.OUT||'lab/16710-validation.json',JSON.stringify({at:new Date().toISOString(),url:URL,rows,checkpoint:true},null,2));
}
await browser.close();
