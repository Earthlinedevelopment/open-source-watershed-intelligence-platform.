...
const outPath='lab/.16605-regional-runtime.mjs';
...
const oldLoop=`    if(!row.regional.pass){row.classification=row.regional.classification;row.pass=false;}\n    else{row.property=await property(frame);row.classification=row.property.classification;row.pass=row.property.pass;}`;
const newLoop=`    if(!row.regional.pass){row.classification=row.regional.classification;row.pass=false;}\n    else{row.classification='PASS';row.pass=true;}`;
if(!src.includes(oldLoop))throw new Error('loop target missing');
