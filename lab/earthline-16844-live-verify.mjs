...
for(let attempt=1;attempt<=8;attempt++){
 ...
 try{
  await page.goto('https://earthlinedevelopment.org/?live16844='+Date.now()+'-'+attempt,{waitUntil:'domcontentloaded',timeout:60000});
  await page.waitForSelector('#searchInput',{timeout:30000});
  const raw=await page.content();const marker=raw.includes('EARTHLINE 16844 — FINAL FINE-SUPPORT REGIONAL CAPACITY');
  ...
  await page.waitForFunction(()=>{const d=window.EARTHLINE_REGIONAL_DISPLAY_AUDIT_16040||window.EARTHLINE_REGIONAL_DISPLAY_AUDIT_16020;return window.EARTHLINE_SUPPORTED_CAPACITY_16843?.build==='EARTHLINE 16844'&&window.EARTHLINE_CORRIDOR_PUBLICATION_AUDIT_16167?.generated>0&&Number(d?.swaleLines||0)>0;},null,{timeout:110000,polling:100});
  ...
  final.attempt=attempt;await page.screenshot({path:'out/16844-live-arkansas.png',fullPage:false});await page.close();break;
 }catch(e){lastError=String(e?.message||e);await page.close();if(attempt<8)await new Promise(r=>setTimeout(r,6000));}
}
await browser.close();fs.writeFileSync('out/16844-live-verify.json',JSON.stringify({at:new Date().toISOString(),final,lastError},null,2));console.log(JSON.stringify({final,lastError}));
if(!final||final.build!=='EARTHLINE 16844'||final.generated<185||final.generated!==final.visible||final.unsafe!==0||final.outside!==0||!final.waterReady||!Number.isFinite(final.coreMs)||final.coreMs>15000||final.localFailed!==0)process.exit(2);
