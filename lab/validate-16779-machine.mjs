...
for(const stateName of STATES){
  ...
  try{
    await page.goto(URL+'?m4516779='+encodeURIComponent(stateName)+'-'+Date.now(),{waitUntil:'domcontentloaded',timeout:60000});
    ...
    await page.waitForFunction(expected=>{
      ...
      return identityMatches && (!!err || (spatial?.build==='EARTHLINE 16779'&&generated>0&&visible===generated));
  ...
  const snap=await page.evaluate(()=>{
    ...
    const fine=window.EARTHLINE_FINE_DISPERSION_REBALANCE_16779||{};
    return {
      ...
      coveragePassed:coverage.passed===true,coverageUnresolved:Array.isArray(coverage.unresolved)?coverage.unresolved:[],
      fineInitialCoverage:Number(fine.initialCoverage??NaN),fineFinalCoverage:Number(fine.finalCoverage??NaN),fineSwaps:Number(fine.swaps||0)
    };
  }).catch(()=>({}));
  const failures=[];
  if(timedOut)failures.push('timeout');
  if(error)failures.push('error '+JSON.stringify(error));
  if(snap.build!=='EARTHLINE 16779')failures.push('build '+snap.build);
  ...
  if(!snap.coveragePassed||snap.coverageUnresolved.length)failures.push('coverage unresolved '+JSON.stringify(snap.coverageUnresolved));
...
fs.writeFileSync(process.env.OUT||'machine16779.json',JSON.stringify(out,null,2));
