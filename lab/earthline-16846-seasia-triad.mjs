...
async function runCandidate(target,loc){
  ...
  const snap=await page.evaluate(({target,harnessError})=>{
    ...
    return {target,harnessError,error:err&&String(err.error||err.message||err),code:pkg?.countryCode||null,package:!!pkg,boundaryCapability:boundary?.capability||boundary?.source||null,
      ...
      waterPaths:Number(run?.derived?.waterPaths||0),directionArrows:Number(run?.derived?.directionArrows||0),watershedFeatures:Number(run?.watersheds?.features||0),watershedStatus:run?.watersheds?.status||null,
      aquiferFeatures:Number(run?.aquifers?.features||0),aquiferStatus:run?.aquifers?.status||null,aquiferSource:run?.aquifers?.source||null,aquiferBoundaryClass:run?.aquifers?.boundaryClass||null,hydrogeologicContext:run?.hydrogeologicContext||null,
      coveragePassed:coverage?coverage.passed!==false:null,aquiferTextVisible:/AQUIFER EVIDENCE/i.test(body),aquiferWordVisible:/AQUIFER/i.test(body),bodyStatus:(body.match(/AQUIFER[^\n]{0,160}/i)||[])[0]||null};
  },{target,harnessError});
...
await browser.close();
const bad=rows.filter(r=>r.resolverError||!r.resolved||String(r.resolved.placeType||'').toLowerCase()!=='country'||!r.candidate||r.candidate.harnessError||r.candidate.error||!r.candidate.package||!r.candidate.boundaryCapability||r.candidate.outside!==0||r.candidate.unsafe!==0||r.candidate.generated!==r.candidate.visible||r.candidate.generated===0||r.candidate.waterPaths===0||r.candidate.totalMs>15000||!r.candidate.runSpan||!r.candidate.countrySpan||Math.abs(r.candidate.runSpan[0]-r.candidate.countrySpan[0])>.02||Math.abs(r.candidate.runSpan[1]-r.candidate.countrySpan[1])>.02);
const review=rows.map(r=>({target:r.target,generated:r.candidate?.generated??null,totalMs:r.candidate?.totalMs??null,waterPaths:r.candidate?.waterPaths??null,directionArrows:r.candidate?.directionArrows??null,watershedFeatures:r.candidate?.watershedFeatures??null,watershedStatus:r.candidate?.watershedStatus??null,aquiferFeatures:r.candidate?.aquiferFeatures??null,aquiferStatus:r.candidate?.aquiferStatus??null,aquiferSource:r.candidate?.aquiferSource??null,aquiferTextVisible:r.candidate?.aquiferTextVisible??null,hydrogeologicContext:r.candidate?.hydrogeologicContext??null}));
