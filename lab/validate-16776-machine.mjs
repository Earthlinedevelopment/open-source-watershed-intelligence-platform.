...
for(const stateName of STATES){
  ...
  try{
    await page.goto(URL+'?m4516776='+encodeURIComponent(stateName)+'-'+Date.now(),{waitUntil:'domcontentloaded',timeout:60000});
    ...
    await page.waitForFunction(expected=>{
      ...
      return identityMatches && (!!err || (spatial?.build==='EARTHLINE 16776'&&generated>0&&visible===generated));
  ...
  if(snap.build!=='EARTHLINE 16776')failures.push('build '+snap.build);
...
fs.writeFileSync(process.env.OUT||'machine16776.json',JSON.stringify(out,null,2));
