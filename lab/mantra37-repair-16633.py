...
if start < 0 or end < 0:
    raise SystemExit('Expected 16584 Regional validity owner markers not found')

replacement = r'''  /* EARTHLINE 16633 — EXACT PREFIX-PRUNED SAMPLE WALK.
     Same 16584 validity owner, same validity mask, same 4-samples-per-grid-cell density,
     same Math.round sample verdict. The existing 16632 prefix index now prunes clear
     sample intervals recursively: if an interval's endpoint-bounded grid rectangle has
     zero invalid cells, every exact sample in that interval is proven valid and skipped.
     Intervals whose rectangle contains invalid cells are subdivided until the original
     sample points are checked verbatim. Out-of-bounds inputs retain the exact prior path.
     No new mask, source, rule, renderer, listener, publication owner, or lifecycle owner. */
    ...
    if(inBounds16584){
      ...
      if(fast16584){
        ...
        while(stack16584.length){
          const range16584=stack16584.pop(),lo16584=range16584[0],hi16584=range16584[1];
...
if marker not in s:
    raise SystemExit('16633 marker missing')

p.write_text(s, encoding='utf-8')
print('16633 exact prefix-pruned sampler written')
