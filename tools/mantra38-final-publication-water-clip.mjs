...
if(!s.includes('function earthlineFinalWaterClip16584(')){
  const helper=`  function earthlineFinalWaterClip16584(hy16584,flows16584,mp16584,runToken16584){
    ...
    window.EARTHLINE_STYLE_WATER_INDEX_16584={
      build:'EARTHLINE 16584',runToken:runToken16584,phase:'final-publication',
      featureCount:Number(index16584.featureCount||0),partCount:Number(index16584.partCount||0),
      sourceCount:Number(index16584.sourceCount||0),binCount:Object.keys(index16584.bins||{}).length,
...
if(!s.includes('flows=earthlineFinalWaterClip16584(hy,flows,m,runToken);')){
  const gate='    window.EARTHLINE_REGIONAL_ATOMIC_PREFLIGHT_16329={';
  const gateCount=s.split(gate).length-1;
  if(gateCount!==1)throw new Error('expected one atomic publication gate, found '+gateCount);
  const clip=`    if(!focusMode&&hy&&hy.validityMask16584){\n      flows=earthlineFinalWaterClip16584(hy,flows,m,runToken);\n    }\n\n`;
  s=s.replace(gate,clip+gate);
}

fs.writeFileSync(path,s);
console.log('Final 16584 serving-water containment now refreshes after the existing camera-settle gate and immediately before atomic Regional publication; no new listener, renderer, camera, idle, polling, source, layer, or verdict owner added.');
